...
description: Switch Sila state tree to a partitioned binary tree
...
discussions-to: https://sila-magicians.org/t/sip-8297-partitioned-binary-tree/28776
status: Draft
...
state read during SVM execution.
...
tosilaer is co-located under one shared key prefix ("stem") to reduce
...
sub-index byte. The zone byte and the tree position tosilaer are a key's
stem:
...
32 zero bytes it would have held, so the SVM cannot distinguish the two. A
...
carrying a rule that belongs to the SVM's value semantics, and a client is
...
trigger because the SVM has no other spelling of "unset": a storage slot is a
...
balance and no code, which the SVM cannot distinguish from a nonexistent
...
The main breaking change is that the tree structure change breaks in-SVM
...
The change is invisible to the SVM. Contracts address storage by 256-bit slot
numbers through `SLOAD` and `SSTORE` and never see tree keys. Key derivation runs
inside the client, below the SVM, exactly as the MPT already hashes slot keys and
