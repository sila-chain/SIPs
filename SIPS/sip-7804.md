...
discussions-to: https://sila-magicians.org/t/sip-7804-withdrawal-credential-update-request/21514
status: Draft
...
complex (based on the experience with the Sil1 bridge).
...
def process_withdrawal_credential_update_request(state: BeaconState, withdrawal_credentials_update_requests: WithdrawalCredentialUpdateRequest) -> None:
    ...
    is_sil1_type = credential_type == SIL1_ADDRESS_WITHDRAWAL_PREFIX
    is_compounding_type = credential_type == COMPOUNDING_WITHDRAWAL_PREFIX
    # Verify valid type
    if not (is_sil1_type or is_compounding_type):
        return;
...
def update_withdrawal_credentials(state: BeaconState, index: ValidatorIndex, new_credential_type: Bytes1, new_withdrawal_credentials: ExecutionAddress) -> None:
    ...
    if (old_credential_type == SIL1_ADDRESS_WITHDRAWAL_PREFIX and new_credential_type == COMPOUNDING_WITHDRAWAL_PREFIX) {
