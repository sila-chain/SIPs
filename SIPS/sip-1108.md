...
For zk-SNARK based protocols on Sila, SIP-1108 will not only reduce the gas costs of verifying zk-SNARKs substantially, but can also aid in [batching tosilaer multiple zk-SNARK proofs](https://github.com/matter-labs/Groth16BatchVerifier). This is also a technique that can be used to split up monolithic zk-SNARK circuits into a batch of zk-SNARKs with smaller individual circuit sizes, which makes zk-SNARKs both easier to construct and deploy.
...
### Parity and Sila already have fast algorithms that justify reduced gas costs  

This SIP does not require Parity or Sila to deploy new cryptographic libraries, as fast bn128 algorithms have already been integrated into these clients. This goal of proposing this SIP for Istanbul, is to supplement [SIP-1829](./sip-1829.md) (arithmetic over generic elliptic curves), providing an immediate solution to the pressing problem of expensive cryptography, while more advanced solutions are developed, defined and deployed.
...
@vbuterin independently proposed a similar reduction after this SIP was originally created, with similar rationale, as [sila-chain/SIPs#1187](https://github.com/sila-chain/SIPs/issues/1187).
