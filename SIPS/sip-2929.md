...
discussions-to: https://sila-magicians.org/t/sip-2929-gas-cost-increases-for-state-access-opcodes/4558
status: Final
...
Data on [txs per day](https://silerscan.io/chart/tx) and [blocks per day](https://silerscan.io/chart/blocks) from Silerscan gives ~160 transactions per block (reference date: Jul 1), implying a large portion of those accesses are just the `tx.sender` and `tx.to` which are excluded from gas cost increases, though likely less than 320 due to duplicate addresses.
...
A WIP early-draft implementation for Gsil can be found here: https://github.com/holiman/go-sila/tree/access_lists
