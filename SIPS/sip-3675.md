...
description: Specification of the consensus mechanism upgrade on SilaMainnet that introduces Proof-of-Stake
author: Mikhail Kalinin (@mkalinin), Danny Ryan (@djrtwo), Vitalik Buterin (@vbuterin)
discussions-to: https://sila-magicians.org/t/sip-3675-upgrade-consensus-to-proof-of-stake/6706
status: Final
...
* **`TERMINAL_TOTAL_DIFFICULTY`** The amount of total difficulty reached by the network that triggers the consensus upgrade. SilaMainnet configuration **MUST** have this parameter set to the value `58750000000000000000000`.
...
* Remove verification of the block's **`nonce`** and **`mixHash`** values with respect to the Silash function.
...
Particularly, this is important for those smart contracts that verify Merkle proofs of transaction/recsipt inclusion and state by validating the hash of externally provided block header against the corresponding value returned by the `BLOCKHASH` operation.
...
The design of the consensus upgrade specified by this document does not introduce backward incompatibilities for existing applications and services built on top of Sila except for those that are described in the [SVM](#evm) section below or heavily depends on the PoW consensus in any other way.


### SVM

Although this SIP does not introduce any explicit changes to the SVM there are a couple of places where it may affect the logic of existing smart contracts.
...
*Note*: Acceleration in the second case is considered for the most extreme of scenarios because it will result in a non-trivial liveness failure on SilaMainnet.
