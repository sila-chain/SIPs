...
description: Specification of the consensus mechanism upgrade on Sila SilaMainnet that introduces Proof-of-Stake
...
The design of the consensus upgrade specified by this document does not introduce backward incompatibilities for existing applications and services built on top of Sila except for those that are described in the [SVM](#svm) section below or heavily depends on the PoW consensus in any other way.
...
Specification of weak subjectivity checkpoints can be found in the `sila-chain/consensus-specs` repository.
