...
discussions-to: https://sila-magicians.org/t/sip-7843-slotnum-opcode/22234
status: Draft
...
The `SLOTNUM` opcode should not increase the complexity of proving SVM execution, as it is similar to existing opcodes such as `TIMESTAMP`. The slot number is included in the block header rather than as a `engine_newPayload` parameter, ensuring that the block is self-contained for proving; no extra inputs to the circuit are required.
