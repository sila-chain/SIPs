...
The frame's `limits.execution` counts toward `MAX_VERIFY_GAS`, tosilaer with the declared execution limits of the other frames in the validation prefix and the cost of checking the transaction signatures. This uses SIP-8141's existing validation budget. SIP-8141 can change that budget later if needed.
