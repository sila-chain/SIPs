...
discussions-to: https://sila-magicians.org/t/sip-8272-recent-roots-for-frame-transactions/28621
status: Draft
...
All calls use ordinary SVM execution and gas accounting.
...
The frame consumes ordinary SVM gas. Its target access and storage reads follow the normal warm and cold access rules, and its data follows the normal SIP-8141 transaction data pricing. This SIP adds no special intrinsic gas, warming rule, or exemption from block gas.
...
The frame MUST execute successfully. During public mempool validation, a client MAY evaluate this frame directly instead of executing `RECENT_ROOT_CODE`, even when other frames in the validation prefix require simulation. This direct evaluation MUST produce the same frame receipt and gas use as SVM execution. It MUST also apply the same warm account and storage key updates and rollbacks as SVM execution. A client MUST reject the transaction if SVM execution would revert or halt exceptionally, including when `frame.limits.execution` is insufficient.

For SIP-8141's public mempool checks on declared limits and total validation work, clients MUST exclude the recent root verifier frame:
...
This exclusion changes only public mempool admission accounting. The frame's declared execution limit remains part of the SIP-8141 transaction maximum cost and block execution gas reservation, and the payer is charged for the frame's actual gas use under the ordinary SIP-8141 rules.
...
| Direct evaluation instead of SVM execution | identical frame receipt, gas use, and warm access sets | same admission result |
