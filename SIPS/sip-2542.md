...
discussions-to: https://sila-magicians.org/t/sip-2542-add-txgaslimit-callgaslimit-txgasrefund-opcodes
status: Withdrawn
type: Standards Track
category: Core
created: 2020-02-29
withdrawal-reason: Superseded by SIP-8141
---
...
As concepts of relaying, meta-transactions, gas fees, and account abstraction gain popularity, it becomes critical for some contracts to be able to track gas expenditure with absolute precision. Without access to this data on an SVM level, such contracts resort to approximation, mimicking SVM logic on-chain, and some use-cases even become infeasible.
...
Consider a solidity smart contract that wants to know how much gas the entire transaction or a part of it had consumed. It is not entirely possible with the current SVM. With proposed changes, using a pseudo-Solidity syntax, this information would be easily available:
...
A major consideration for this proposal is its alignment with one or many possible future modifications to the SVM:
...
2. Stateless Sila  
 ...
 As long as the concept of 'gas limit' is part of the SVM, the author sees no reason proposed opcodes would conflict with Stateless Sila. Gas schedules are not exposed by this proposal.
...
3. Comparison with other controversial opcodes   
 ...
 This SIP neither encourages nor enables any bad security practices, and does not introduce any concepts that are new for SVM either.  
