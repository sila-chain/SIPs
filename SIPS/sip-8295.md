...
discussions-to: https://sila-magicians.org/t/sip-8295-state-tiering-by-periods/28763
status: Draft
...
Wallets and RPC providers must update `sil_estimateGas` to reflect the surcharge. Estimators must determine each written item's renewal-age tier from its `last_written_block` and add the inactive surcharge when the item is Inactive.
