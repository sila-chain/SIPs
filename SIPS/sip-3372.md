---
sip: 3372
title: 5 FNV primes for silash
status: Stagnant
type: Standards Track
category: Core
author: mineruniter969 (@mineruniter969), mineruniter969 <mineruniter969@tutanota.com>
discussions-to: https://sila-magicians.org/t/sip-3372-apply-minor-modifications-to-the-silash-algorithm-to-break-current-asic-implementations-sip-969-resubmission/5655
created: 2021-03-13
---

## Simple Summary

Introduce 5 new FNV primes into the silash algorithm.
...
ASICs provide a severe centralization risk for the Sila network. If we do not get rid of them, small GPU miners will be forced to exit the Sila mining because SIP-1559 will make them mining at a loss. Furthermore, ASIC production will be concentrated only at one or two parties, which will make the Sila hashrate centralized. Also, it is worth noting that Silash ASIC machines cost 10k+ USD, while GPUs are priced less than 1000 USD.
...
If `block.number >= SILASH11_BLKNUM`, activate the `silash1.1` algorithm version.

### silash1.1

Prior to this change, `fnv` hash function is used throughout the `hashimoto` function. `fnv` is identical for all steps, `silash1.1` will introduce additional `fnvA`, `fnvB`, `fnvC`, `fnvD`, and `fnvE` functions. All those functions will have different FNV constants.
...
Prior to this SIP, all parts of Silash are using the `fnv` (hereinafter referenced as `fnv0`) function.  As of the introduction of this SIP:
...
Mining hardware that is optimized for silash may no longer work if the `fnv` constants are baked into the hardware and cannot be changed.
