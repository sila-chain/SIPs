...
clients observed a choice sequence of lowest total difficulty followed by first observed block. Gsil
added one extra rule of preferring the shortest chain before preferring the first observed block.
This fork would have resolved itself with Gsil's rule, but there is still a configuration where the
...
In this case both forks have equal total difficulty and equal length. So Gsil's rule would not break
