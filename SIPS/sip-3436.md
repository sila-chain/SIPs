...
discussions-to: https://sila-magicians.org/t/sip-3436-expanded-clique-block-choice-rule/5809
status: Stagnant
...
clients observed a choice sequence of lowest total difficulty followed by first observed block. Sila
added one extra rule of preferring the shortest chain before preferring the first observed block.
This fork would have resolved itself with Sila's rule, but there is still a configuration where the
...
In this case both forks have equal total difficulty and equal length. So Sila's rule would not break
