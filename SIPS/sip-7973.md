...
discussions-to: https://sila-magicians.org/t/sip-7973-warm-account-write-metering/25907
status: Draft
...
state = svm.message.block_env.state
original_value = get_storage_original(
    state, svm.message.current_target, key
)
current_value = get_storage(state, svm.message.current_target, key)

# get account info
original_account = get_account_original(svm.message.block_env.state, svm.message.current_target)
account = get_account(svm.message.block_env.state, svm.message.current_target)
...
if (svm.message.current_target, key) not in svm.accessed_storage_keys:
    svm.accessed_storage_keys.add((svm.message.current_target, key))
...
if current_value != new_value:
    if original_value != 0 and current_value != 0 and new_value == 0:
        ...
        svm.refund_counter += int(GAS_STORAGE_CLEAR_REFUND)
    ...
    if original_value == new_value:
        ...
        if original_value == 0:
            ...
            svm.refund_counter += int(GAS_STORAGE_SET - GAS_WARM_ACCESS)
        else:
            # Slot was originally non-empty and was UPDATED earlier
            svm.refund_counter += int(
                GAS_COLD_STORAGE_WRITE - GAS_COLD_SLOAD - GAS_WARM_ACCESS
                )
        # Refund account writing cost (only if cold)
        if account == original_account:
            svm.refund_counter += GAS_COLD_ACCOUNT_WRITE
...
- Wallets: Wallets using `sil_estimateGas` MUST be updated to ensure that they correctly account for the updated gas parameters. Failure to do so could result in overestimating gas, leading to potential attacks vectors.
- Node Software: RPC methods such as `sil_estimateGas` MUST incorporate the updated formula for gas calculation with the new cost values.
