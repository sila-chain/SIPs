...
discussions-to: https://sila-magicians.org/t/anti-sil-asic-mining-sip-1488-pr/1807
status: Stagnant
...
## Simple Summary
This SIP modifies silash in order to break ASIC miners specialized for the current silash mining algorithm.
...
Following TSilashV1 algorithm suggests safe transition of PoW algorithms and secure the FNV Algorithm in MIX Parts.

## Motivation
Provide original Silash proof of work verification with minimal set of changes by updating FNV0 algorithm

## Specification

#### 1. Reference materials on SILASH FNV0

#### Where FNV Applied on SILASH

- In [SILASH](https://web.archive.org/web/20200505215203/https://github.com/sila/wiki/wiki/Silash), FNV Hash is used on
...
#### 2. Current applied Silash FNV hash implementation is deprecated now.
...
Consider real computation resources, in TSilashV1 uses hash byte_of_data to 4bytes aligned data.

In TSILashV1, Adapts fully follow the FNV1A implementation.

  - TSILASHV1 FNV1A implementation

Following are reference implementation of FNV1A adapted in TSILashV1.
...
FNV Mix Algorithm Analysis for TSilashV1
...
In case of silash algorithm, it can't prevent ASIC forever.

And, current silash algorithm's FNV function is deprecated.

So, It needs to be upgraded and it will make current silash based ASICs obsolete.

And current TSILASHV1 FNV1A implementation is based on most of silash , which is verified for a long time.

Another propose of big differencing the Silash algorithm need to crypto analysis for a long times and need to GPU code optimization times.

**Verification and Optimization timeline Examples**

original silminer (2015) -> claymore optimized miner (2016) [1year]

genoil silminer (2015) -> sila-mining/silminer (2017) [2year]
