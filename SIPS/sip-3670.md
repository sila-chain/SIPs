...
discussions-to: https://sila-magicians.org/t/sip-3670-eof-code-validation/6693
status: Stagnant
...
Currently existing contracts require no validation of correctness and SVM implementations can decide
...
Moreover, SVM implementations may require fewer paths to decide which instruction is valid in
...
Allowing implicit zero immediate data for `PUSH` instructions introduces inefficiencies to SVM implementations without any practical use-case (the value of a `PUSH` instruction at the code end cannot be observed by SVM). This SIP requires all immediate bytes to be explicitly present in the code.
