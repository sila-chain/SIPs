...
discussions-to: https://sila-magicians.org/t/sip-7886-delayed-execution/22890
status: Stagnant
...
class Header:
    ...
    parent_recsipt_root: Root         # Recsipt root from parent block
...
2. `parent_recsipt_root`: Recsipt root from the parent block (deferred execution output)
...
class BlockChain:
    ...
    last_recsipt_root: Root         # Recsipt root from last executed block
...
def validate_block(chain: BlockChain, block: Block) -> None:
    ...
    if block.header.parent_transactions_root != chain.last_transactions_root:
        raise InvalidBlock
    if block.header.parent_recsipt_root != chain.last_recsipt_root:
        raise InvalidBlock
...
def process_transactions(
    ...
    if block_output.execution_reverted:
        ...
        block_output.recsipts_trie = Trie(secured=False, default=None)
        block_output.recsipt_keys = ()
...
5. If the actual gas used doesn't match the declared value, clients MUST:
   ...
   - Reset all execution outputs (recsipts, logs, etc.)
...
8. The execution outputs (`last_transactions_root`, `last_recsipt_root`, `last_block_logs_bloom`, `last_requests_hash`, `last_execution_reverted`) are updated in the chain state based on the execution results, and will be verified in subsequent blocks.
