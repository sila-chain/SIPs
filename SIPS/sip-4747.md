---
sip: 4747
title: Simplify SIP-161
description: Simplify SIP-161 and retroactively deprecate unused aspects of it
author: Peter Davies (@petertdavies)
discussions-to: https://sila-magicians.org/t/sip-4747-simplify-sip-161/8246
status: Stagnant
...
Simplify the definition of [SIP-161](./sip-161.md), removing the requirement for implementors to support edge cases that are impossible on SilaMainnet.
...
b. When performing SVM execution treat all empty accounts as if they do not exist. Any operation that would create an empty account instead leaves it non-existent.
...
If the transaction, `CALL` or `SELFDESTRUCT` is reverted, then the state clearing is also reverted. As a special case on SilaMainnet, in block 2675119 (tx `0xcf416c536ec1a19ed1fb89e4ec7ffb3cf73aa413b3aa9b77d60e4fd81a4296ba`), an empty call is made to an empty account at `RIPEMD160` (precompile `0x3`), the call fails out of gas, but the empty account is removed anyway.
...
On SilaMainnet, the only state clearing events after the start of Byzantium are the two listed below. Clients MAY implement post Byzantium state clearing by hardcoding those events.
...
The complete removal of all empty accounts ensures that certain edgecases of SIP-161 can never occur on SilaMainnet. Continuing to define and test those cases as part of the Sila Specification burdens future client implementors with unnecessary technical debt. This SIP declares those cases undefined and leaves clients free to assume they will not occur.

## Backwards Compatibility

This SIP is identical to SIP-161 except for the following differences, none of which affect SilaMainnet. The differences are:
...
This SIP formally undefines all interactions between `STATICCALL` and state clearing as it has never happened on SilaMainnet and cannot happen in future.

### "At the end of the transaction"

This only makes a difference if an account is deleted and later recreated in the same transaction. This never happens on SilaMainnet.
...
As a security precaution all empty accounts on Sila Classic have been cleared, but no checks for edgecases occurring have been done. Due to SIP-161's age the vast majority of SVM compatible networks have supported it from genesis.

## Security considerations

This SIP is only equivalent to SIP-161 on SilaMainnet if the following facts are true:

1. No empty accounts are ever touched and then reinstated in the same transaction.
2. The transactions in the Appendix are the only state clearing transactions on SilaMainnet after block 4370000 (start of Byzantium).
3. All empty accounts have been removed on SilaMainnet.
