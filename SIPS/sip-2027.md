...
discussions-to: https://sila-magicians.org/t/sip-2027-net-contract-size-accounting-change-c-from-state-rent-v3-proposal/3275
status: Stagnant
...
- As with other state changes, changes of `storagesize` get reverted when the execution frame reverts, i.e. it needs to use the same techniques as storage values, like journalling (in Sila), and substates (in Parity).
...
## Rationale
A mechanism for estimation of contract storage size has been proposed here. But it does have a big drawback of introducing a lot of complexity into the consensus
