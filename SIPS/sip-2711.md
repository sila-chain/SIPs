...
discussions-to: https://sila-magicians.org/t/sip-2711-separate-gas-payer-from-msg-sender/4353
status: Withdrawn
...
* If any form of dust-account clearing is introduced, e.g. (https://github.com/sila/SIPs/issues/168), it will be necessary to introduce a replay protection, such as https://github.com/sila/SIPs/issues/169 . Having temporal replay protection removes the need to change nonce-behaviour in the state, since transactions would not be replayable at a later date than explicitly set by the user.
...
### New Transaction Recsipt

As of `FORK_BLOCK_NUMBER` an [SIP-2718](./sip-2718.md) transaction recsipt with a `TransactionType` of `2` will have its `Payload` interpreted as a `rlp([status, cumulativeGasUsed, logsBloom, logs][])` where each item of the array corresponds to the child-transaction at matching offset in the transaction type 2 `Payload`.
