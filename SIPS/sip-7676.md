...
discussions-to: https://sila-magicians.org/t/eof-prepare-for-address-space-extension/19537
status: Stagnant
...
Operations in the Legacy SVM trim off the top 12 bytes of an address operand before evaluation. This
...
address size from 20 to 32 bytes"). One issue ground the work to a halt: SVM opcodes that accept
addresses trim all but the lowest 20 bytes out from the operand before processing. SVM Reference
tests verify this behavior in the 'stBadOpcode/invalidAddr.json' tests.

The SVM Object Format presents an opportunity to remove this address masking in a backwards
