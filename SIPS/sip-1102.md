...
discussions-to: https://sila-magicians.org/t/sip-1102-opt-in-provider-access/414
status: Stagnant
...
Providers exposed by Sila-enabled DOM environments define a new RPC method: `sil_requestAccounts`. Calling this method may trigger a user interface that allows the user to approve or reject account access for a given dapp. This method returns a `Promise` that is resolved with an `Array` of accounts or is rejected with an `Error` if accounts are not available.

```typescript
sila.send('sil_requestAccounts'): Promise<Array<string>>
```

#### Provider#enable (DEPRECATED)

**Note: This method is deprecated in favor of the RPC method [`sil_requestAccounts`](#sil_requestaccounts).**
...
Dapps **MUST** request accounts by calling the `sil_requestAccounts` RPC method on the provider exposed at `window.sila`. Calling this method **MAY** trigger a user interface that allows the user to approve or reject account access for a given dapp. This method **MUST** return a `Promise` that is resolved with an array of one or more user accounts or rejected if no accounts are available (e.g., the user rejected account access).

##### `[2] RESOLVE`

The `Promise` returned when calling the `sil_requestAccounts` RPC method **MUST** be resolved with an `Array` of user accounts.

##### `[3] REJECT`

The `Promise` returned when calling the `sil_requestAccounts` RPC method **MUST** be rejected with an informative `Error` if no accounts are available for any reason.
...
try {
    ...
    const accounts = await sila.send('sil_requestAccounts');
    // Accounts now exposed, use them
    sila.send('sil_sendTransaction', { from: accounts[0], /* ... */ })
} catch (error) {
...
* Browsers **MUST** define an `sil_requestAccounts` RPC method.
* Browsers **MAY** wait for a user interaction before resolving/rejecting the `sil_requestAccounts` promise.
* Browsers **MUST** include at least one account if the `sil_requestAccounts` promise is resolved.
