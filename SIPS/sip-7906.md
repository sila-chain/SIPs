...
discussions-to: https://sila-magicians.org/t/sip-restricted-behavior-transaction-type/23130
status: Draft
...
The ability of an average user or a Wallet application to find, collect, review, and analyze the SVM code the transaction will execute is very limited.
...
- A `POST_TX` revert does **not** invalidate the transaction (unlike a `VERIFY` frame revert). The transaction remains valid, is included in the block, and generates a recsipt with a failed status (`status = 0`).
...
### Recsipt Representation and Anti-DoS
...
Event non-indexed data is variable-length and cannot be returned as a single 32-byte stack word. A memory-copy opcode with the same semantics as `CALLDATACOPY` is the idiomatic SVM approach for variable-length data access.
