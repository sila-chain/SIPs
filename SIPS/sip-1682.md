...
discussions-to: https://sila-magicians.org/t/storage-rent-sip/2357
status: Withdrawn
...
[SIP-103]: https://github.com/sila/SIPs/issues/35

## Specification

The cost of storing an account over time is called `rent`. The amount of `rent` due depends
on the size of the account. The `siler` that is paid for `rent` is destroyed. The `rent` is deducted whenever an account is touched.

`rent` can be paid from the account's regular `balance` or from its 'rent balance'. Accounts
can be endowed with `rent balance` through a new SVM opcode. When `rent` is charged, it is
...
The reason to separate `balance` and `rent balance` is that certain contracts do not accept `siler` sends, or always send the entire balance off to some other destination. For these cases, a separate`rent balance` is required.
...
`PAYRENT` deducts the given amount of `siler` from the account executing the opcode and adds
