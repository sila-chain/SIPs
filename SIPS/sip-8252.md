...
discussions-to: https://sila-magicians.org/t/sip-8252-execution-layer-reorg-state-retention-window/28463
status: Draft
type: Informational
created: 2026-04-24
---
...
This SIP does not constrain retention of block headers, bodies, or recsipts; those are governed by history-expiry rules.
...
A deep reorg is processed by rolling state back to the common ancestor with the incoming branch and re-executing forward. The new branch's blocks arrive via the engine API or P2P; the local node does not need to have stored the displaced branch's headers, bodies, or recsipts. What it strictly needs is the ability to reconstruct state at the ancestor, which makes state retention the load-bearing requirement for reorg recovery. Block-data retention is a separate, network-serving concern addressed by history-expiry rules.
