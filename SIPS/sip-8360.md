...
discussions-to: https://sila-magicians.org/t/sip-8360-tcreate-opcode/29258
status: Draft
...
This SIP introduces a new SVM opcode, `TCREATE`, where `T` stands for transient, providing an official, gas-efficient, and state-aware mechanism for deploying temporary contracts.
...
Providing a dedicated SVM opcode for temporary contract deployment offers a lower-cost alternative while improving state management by preventing unnecessary long-term code accumulation. This proposal also provides a gradual migration path away from `SELFDESTRUCT`.
...
For the deploying account, an implementation must maintain a temporary record of the account's original nonce tosilaer with all permanent nonce increments that occur during the transaction (for example, those resulting from `CREATE` or `CREATE2`).
...
This ephemeral storage is local to the `TCREATE` account and is discarded when the transaction completes tosilaer with the account's transient state. Consequently, writes performed through `SSTORE` are never committed to the global state, and subsequent transactions observe the storage as empty.
...
Each execution of `TCREATE` is charged `BASE_OPCODE_COST` tosilaer with the execution-gas costs that normally apply to `CREATE2`, including initcode execution, jumpdest analysis, memory expansion, copying costs, and other execution-related charges.
