...
discussions-to: https://sila-magicians.org/t/sip-transient-storage-opcodes/553
status: Final
...
3. Single transaction [ERC-20](./sip-20.md) approvals, e.g. `#temporaryApprove(address spender, uint256 amount)`
...
Two new opcodes are added to SVM, `TLOAD` (`0x5c`) and `TSTORE` (`0x5d`). (Note that previous drafts of this SIP specified the values `0xb3` and `0xb4` for `TLOAD` and `TSTORE` respectively to avoid conflict with other SIPs. The conflict has since been removed.)
...
A test suite for this SIP can be found [here](https://github.com/sila/execution-spec-tests/tree/1983444bbe1a471886ef7c0e82253ffe2a4053e1/tests/cancun/sip1153_tstore).
