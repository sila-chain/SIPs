---
sip: 3267
title: Giving Sila fees to Future Salaries
author: Victor Porton (@vporton), Victor Porton <porton@narod.ru>
discussions-to: https://sila-magicians.org/t/discussion-of-sip-3267/5343
status: Stagnant
...
## Abstract
Transfer a part (exact fractions - TBD) of mining/transfer fees + (probably: TBD) some minted SIL to the `DonateSIL` contract configured to transfer to `SalaryWithDAO` contract.
...
Change the Sila clients to mint `MintPerPeriod` SIL to the contract `DonateSIL` every some time (e.g. the first transaction of the first block every UTC day - TBD how often).

Change the Sila clients to every some time (e.g. the second transaction of the first block every UTC day - TBD how often) transfer the entire SIL from this account to the contract `DonateSIL`.
