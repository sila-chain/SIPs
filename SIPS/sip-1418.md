...
discussions-to: https://sila-magicians.org/t/sip-1418-storage-rent/10737
status: Stagnant
...
* `PAYRENT(address, amount)` -- Calls `PAYRENT` opcode
  * This is a convenience for humans to send Siler from their accounts and turn it into rent. Note that simple accounts (CODESIZE == 0) cannot call arbitrary opcodes, they can only call CREATE or CALL.
...
Siler converted to rent cannot be converted back. Anybody that works in accounting and knows about gifts cards should tell you this is a good idea. It makes reasoning about the system much easier.
...
The rent is priced in cold, hard Siler. It is not negotiated by clients, it is not dynamic.
...
FYI, there are about 15B words in the SilaMainnet dataset and about 100M total Siler mined. This means if all Siler was spent on storage at current proposed prices it would be 400 terabyte-years of storage. I'm not sure if it is helpful to look at it that way.
