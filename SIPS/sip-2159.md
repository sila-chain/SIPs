...
discussions-to: https://sila-magicians.org/t/common-chain-metrics/3415
status: Final
...
## Specification
The table below defines metrics which may be captured by Sila clients which expose metrics to Prometheus. Clients may expose additional metrics however these should not use the `sila_` prefix.

| Name                             | Metric type | Definition                                                        | JSON-RPC Equivalent                                                 |
|----------------------------------|-------------|-------------------------------------------------------------------|---------------------------------------------------------------------|
| sila_blockchain_height       | Gauge       | The current height of the canonical chain                         | `sil_blockNumber`                                                   |
| sila_best_known_block_number | Gauge       | The estimated highest block available                             | `highestBlock` of `sil_syncing` or `sil_blockNumber` if not syncing |
| sila_peer_count              | Gauge       | The current number of peers connected                             | `net_peerCount`                                                     |
| sila_peer_limit              | Gauge       | The maximum number of peers this node allows to connect           | No equivalent                                                       |

Note that `sila_best_known_block_number` always has a value. When the `sil_syncing` JSON-RPC method would return `false`, the current chain height is used.
...
## References
 ...
 2. Beacon chain metrics specification. https://github.com/sila/sil2.0-metrics/blob/master/metrics.md
