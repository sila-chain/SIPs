...
Calldata is added first, to speed up worst-case payload propagation and expand available SVM gas—without compromising gas introspection. A method for facilitating gas aggregation across resources within the SVM is outlined as an avenue for preserving backward compatibility when expanding further. The logic of SIP-7918 is integrated into the multidimensional setting to ensure that calldata has a higher cost per byte than blob data.
...
def get_required_max_fee(base_fees: list[int], tx_gas_limits: list[int]) -> int:
    ...
    max_evm_base_fee = max(base_fees[i] for i in SVM_INDICES)
