...
description: Let transactions specify one aggregate `max_fee` budget for all resources, unify fee markets, normalize gas, and generalize SIP-7918.
author: Anders Elowsson (@anderselowsson), Vitalik Buterin (@vbuterin), Maria Silva (@misilva73)
discussions-to: https://sila-magicians.org/t/sip-7999-unified-multidimensional-fee-market/25010
status: Draft
...
Calldata is added first, to speed up worst-case payload propagation and expand available SVM gas—without compromising gas introspection. A method for facilitating gas aggregation across resources within the SVM is outlined as an avenue for preserving backward compatibility when expanding further. The logic of [SIP-7918](./sip-7918.md) is integrated into the multidimensional setting to ensure that calldata has a higher cost per byte than blob data.
...
| `SVM_LIMIT_TARGET_RATIO` | `2` | Ratio of SVM gas target to SVM gas limit |
...
def get_max_fee(tx: Transaction) -> int:
    ...
    elif is_sip_1559(tx.type): # SIP-1559 txs have no blobs
...
and the `gas_targets`
  ...
  * `gas_targets[0]` must equal `gas_limits[0] // SVM_LIMIT_TARGET_RATIO`.
...
A resource such as state is more attractive to separate than calldata. We could achieve exact control over state growth, while allowing temporary spikes many times above current gas limits. However, an expansion into other resources requires a multidimensional gas repricing, which has currently not yet been completed. The next section will further discuss remaining complexities of multidimensional SVM gas, and the strategies available for overcoming them.

### Expansion paths for multiple SVM resources

This SIP has been designed to facilitate a future expansion into multiple SVM resources. We conclude by outlining the challenges inherent to such an expansion, and the different paths that it can take.

#### SVM without gas observability

One long-term vision for the SVM is to move away from gas observability. This is one of the features of EOF ([SIP-7692](./sip-7692.md)), e.g., through revamped `CALL` instructions in [SIP-7069](./sip-7069.md) such as `EXTCALL`. The new calls no longer accept a gas stipend as an input parameter, and the SVM instead makes available some reasonable fraction of all gas across dimensions (e.g., 63/64). Legitimate use cases previously handled via gas observability are then instead taken over by, e.g., the `PAY` opcode [SIP-5920](./sip-5920.md). 

For compatibility with legacy code, the SVM can in this scenario reinterpret legacy subcalls with a gas parameter by forwarding the same fraction of the caller's remaining budget in each resource dimension. Concretely, if the call stipulates $g_c$ and the aggregate remaining SVM-gas budget is $g_a$, the callee receives, for each SVM resource with remaining budget $g_r$, the amount $\bigl\lfloor g_r \cdot \min\!\bigl(1,\tfrac{g_c}{g_a}\bigr) \bigr\rfloor$. For completeness, the `GAS` opcode could likewise return, e.g., $g_a$ (the per-call aggregate remaining budget at this point across all resource dimensions). Note, however, that reinterpreting legacy calls and `GAS` in this way can still change the behavior of contracts that rely on precise gas observability or gas-capped subcalls, and such contracts may break.

#### SVM that retains gas observability

It is possible to expand into multiple SVM resource dimensions without breaking existing contracts that rely on gas observability. By treating gas as *fungible across resources* (just as today), a single budget can be forwarded and counted toward any resource that consumes it. It should be noted that old contracts may still break due to repricing of the resources they use; indeed, a gas repricing effort is currently underway in Sila that likely will cause such breakage. But this may be treated as a separate concern. 

#### Transaction types under multiple SVM resources with non-deterministic limits

A key difference between the new and old transaction types is that the old transaction types set only one limit, whereas the new can set several. When adding deterministic resources such as calldata, this is not a concern, because consumption of this resource can be deducted from the user-specified limit as a pre-processing step. But once there is more than one non-deterministic resource—as can be the case when SVM gas is separated into several resources—things get slightly more complicated. If we wish to ensure that old transaction types still can function properly under these circumstances, we must apply the single limit to multiple non-deterministic resources. The following subsections will outline how this can be achieved by aggregating the gas of these separate resources during SVM processing.

The new transaction type can supply several limits, making expansion into multiple SVM resources more straightforward. However, it turns out that we may also wish to retain the ability of the new transaction type to set a single limit for SVM gas. The reason is that the single limit and aggregate processing facilitates backward compatibility for contracts that rely on gas observability. As previously noted, these contracts must be able to supply subcalls with an aggregate gas stipend, to be counted against any SVM resource. Furthermore, there is an inherent simplicity of the single limit that is not to be discounted.

In all considered approaches below, the number of non-deterministic SVM resources still increases at the *block level*. To retain the ability to reject a transaction that may require more gas for a resource than its associated block limit, it becomes necessary to alter the check on `gas_used_so_far`. Specifically, when a transaction stipulates only the main SVM gas limit, then this limit must be conservatively counted toward all underspecified dimensions of `gas_used_so_far` before the limit check:
...
else: # The joint SVM gas limit is expanded to cover all block resources
...
An existing proposal by Inês Silva and Crapis is *Multidimensional gas metering* (not yet submitted as an SIP), which prices gas in one dimension, but applies limits at the block level across several dimensions when updating the (single) SVM base fee. The SVM will thus track multidimensional gas consumption, but perform the limit check on the aggregate, which is also passed on in subcalls. The proposed SIP could be combined with Multidimensional gas metering, for example by using a single SVM gas and calculating metered block gas usage separately, before submitting the outcome as a single resource dimension to the revamped `calc_excess_gas()`.

#### Multidimensional fee market with aggregate SVM gas

To promote full resource utilization while maintaining backward compatibility for subcalls and old transaction formats, we could instead use *separate* gas prices for each SVM resource, while retaining within the SVM the ability to *aggregate the gas* consumption across resources. We refer to this as a *Multidimensional fee market with aggregate gas*. In the fully aggregated scenario, the user stipulates a single `gas_limit` for SVM gas, and the protocol must make sure that the `max_fee` covers the maximum possible fee, from consuming `gas_limit` of the SVM resource that has the most expensive base fee. Execution is then fully backward compatible, and the SVM operates the same way as with Multidimensional gas metering.
...
#### Multidimensional fee market with hybrid SVM gas

In a *Multidimensional fee market with hybrid SVM gas*, the user has separate options: 

1. Provide a single gas limit for the SVM resources. The SVM can then operate as in the aggregate gas model, after the initial restrictive fee allocation check.
2. Provide a gas limit for each SVM resource. Capital efficiency is then preserved. The SVM operates without gas observability as previously outlined, with the gas parameter of old subcalls converted to a multidimensional counterpart. The transaction is during processing checked against its multiple limits, as opposed to the aggregate. 

It is further possible to give the user the freedom to stipulate *some* SVM gas limits, but not all. The user can for example set the gas limit to 0 for any resource it knows that it will not use, instead of not setting a gas limit for that resource. The aggregation then takes place only across resources without an individual stipulated limit, reducing the required `max_fee` allocation while preserving full/a higher level of backward compatibility. These features come at a cost of somewhat increased complexity.

#### Specification for aggregate and hybrid SVM gas

For the hybrid model, the `get_gas_limits()` function must be updated to alternatively return multiple SVM gas limits, in the case the user stipulates the full list.
...
For the aggregate model, the `get_required_max_fee()` must be updated such that the protocol applies the worst-case SVM base fees across the SVM gas. The new constant `SVM_INDICES` specifies the SVM resource indices. Note that `TX_BASE_COST` is treated as a deterministic component of the SVM gas to reduce required capital allocation. Note that even when pursuing Options 1-2 for aggregate gas, the aggregation step is still applied in consideration of SIP-7805.
...
def get_required_max_fee(base_fees: list[int], tx_gas_limits: list[int]) -> int:
    ...
    determ_svm_cost = TX_BASE_COST * base_fees[0]
    require(tx_gas_limits[0] >= TX_BASE_COST)   # Limit MUST cover TX_BASE_COST.
    variable_svm_limit = tx_gas_limits[0] - TX_BASE_COST
    # 2. Determine SVM cost based on most expensive resource
    max_svm_base_fee = max(base_fees[i] for i in SVM_INDICES)
    svm_cost = determ_svm_cost + variable_svm_limit * max_svm_base_fee
    # 3. Return total SVM cost + blob cost + calldata cost
    return svm_cost + base_fees[1]*tx_gas_limits[1] + base_fees[2]*tx_gas_limits[2]
```

For the hybrid model, the line stipulating `tip_indices` in `get_max_priority_fee()` must be updated to include all new SVM resources `tip_indices = [2] + SVM_INDICES`. It is also feasible to adjust the functioning of legacy transactions, to let them tip only according to the headroom between the SVM resource with the highest base fee and the `gasprice`. The aim would be to prevent them from having to pay an excessive tip when the `gasprice` was set high merely to cover for the SVM resource with the highest base fee (of which they use little). The tip would then be calculated as the premium above the highest SVM resource base fee.

To allow transactors to specify *some* limits, the `tx.gas_limit` list must instead have a nested format, where `gas_limit[0]` is the aggregate limit for unspecified SVM resources, and `[index, limit]` pairs then follow: `tx.gas_limit = [aggregate, [[index, limit], [index, limit],...]]`. The protocol then computes the required max fee from resources with both specified and unspecified limits. The maximum base fee among the resources with unspecified limits is in this case multiplied with the aggregate, and this fee is summed tosilaer with the fees of specified limits.
...
Backward compatibility for contracts that rely on gas introspection can be resolved by giving users the ability to rely on aggregated SVM gas, while still potentially pricing SVM resources separately. Wallets must be updated to handle multidimensional gas accounting with several base fees.
