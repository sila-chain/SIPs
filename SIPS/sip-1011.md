...
discussions-to: https://github.com/djrtwo/SIPs/issues/5
created: 2018-04-20
---
...
Through the use of Siler deposits, slashing conditions, and a modified fork choice, FFG allows the underlying PoW blockchain to be finalized.  As network security is greatly shifted from PoW to PoS, PoW block rewards are reduced.

This SIP does not contain safety and liveness proofs or validator implementation details, but these can be found in the [Casper FFG paper](https://arxiv.org/abs/1710.09437) and [Validator Implementation Guide](https://github.com/sila/casper/blob/master/VALIDATOR_GUIDE.md) respectively.
...
* **validator**: A participant in the Casper FFG consensus that has deposited siler in the casper contract and has the responsibility to vote and finalize checkpoints.
...
See the [Validator Implementation Guide](https://github.com/sila/casper/blob/master/VALIDATOR_GUIDE.md) for validator details.

#### MSG_HASHER_CODE

The source code for `MSG_HASHER_CODE` is located [here](https://github.com/sila/casper/blob/master/casper/contracts/msg_hash.se.py).
The source is to be migrated to Vyper LLL before the bytecode is finalized for this SIP.

The SVM init code is:
```
TBD
```

The SVM bytecode that the contract should be set to is:
```
TBD
```
...
The source code for `PURITY_CHECKER_CODE` is located [here](https://github.com/sila/research/blob/master/impurity/check_for_impurity.se).
The source is to be migrated to Vyper LLL before the bytecode is finalized for this SIP.
...
[here](https://github.com/sila/casper/blob/master/casper/contracts/simple_casper.v.py).
The contract is to be formally verified and further tested before the bytecode is finalized for this SIP.

The SVM init code with the above specified params is:
```
TBD
```
...
The finality gadget is designed to minimize changes across clients. For this reason, FFG is implemented within the SVM, so that the contract byte code encapsulates most of the complexity of the fork.

Most other decisions were made to minimize changes across clients. For example, it would be possible to allow `CASPER_ADDR` to mint Siler each time it paid rewards (as compared to creating the contract with `CASPER_BALANCE`), but this would be more invasive and error-prone than relying on existing SVM mechanics.

#### Deploying Casper Contract
The `MSG_HASHER_CODE` and `PURITY_CHECKER_CODE` both do not require any initialization so the SVM bytecode can simply be placed at `MSG_HASHER_ADDR` and `PURITY_CHECKER_ADDR`. On the other hand, the casper contract _does_ require passing in parameters and initialization of state. This initialization would normally occur by the SVM init code interacting with the CREATE opcode. Due to the nature of this contract being deployed outside of normal block transactions and to a particular address, the SVM init code/CREATE method requires client specific "hacks" to make it work. For simplicity of specifying across clients, the SVM bytecode -- `CASPER_CODE` -- is placed at `CASPER_ADDR` followed by an explicit `CALL` to a one-time `init` method on the casper contract. `init` handles all of the logic that a constructor normally would, accepting contract parameters as arguments and setting initial variable values, and can only be run _once_.
...
For more details on validator account abstraction, see the [Validator Implementation Guide](https://github.com/sila/casper/blob/master/VALIDATOR_GUIDE.md).
