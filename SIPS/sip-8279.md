...
description: Meter SIP-7928 Block Access List bytes at runtime and fold them into the SIP-7623 transaction floor.
author: Toni Wahrstätter (@nerolation)
discussions-to: https://sila-magicians.org/t/sip-8279-block-access-list-byte-floor/28662
status: Draft
...
`tx_env.static_floor` holds this value and `tx_env.floor_gas_used` is initialised to it (with `bal_data_bytes = 0`) at the start of execution. The per-auth term covers each authorization's worst-case BAL contribution statically, so `set_delegation` (which runs before the SVM's OOG handler) never meters at runtime.
...
Wallets and `sil_estimateGas` MUST compute the new static floor (including the per-auth term) and, when tracing execution, meter the runtime BAL bytes for cold accesses, value-bearing calls, and deployments alongside intrinsic gas.

## Test Cases

### Bare SIL transfer

`tx.to = recipient`, `tx.value = 1 siler`, `tx.data = b""`.
...
Wallets and `sil_estimateGas` MUST simulate `floor_gas_used` alongside intrinsic gas. Without it, estimates for floor-bound transactions are too low and submissions will be rejected at validation.
