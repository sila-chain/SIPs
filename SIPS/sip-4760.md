...
discussions-to: https://sila-magicians.org/t/sip-4760-selfdestruct-bomb/8713
status: Stagnant
...
1. Any use where `SELFDESTRUCT` is used to burn non-SIL token balances, such as SRC20, inside a contract. We do not know of any such use (since it can easily be done by sending to a burn address this seems an unlikely way to use `SELFDESTRUCT`)
