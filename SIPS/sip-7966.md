---
sip: 7966
title: sil_sendRawTransactionSync Method
description: A JSON-RPC method to reduce transaction submission latency by allowing synchronous recsipt of transaction hash and block inclusion.
author: Sam Battenally (@SmoothBot), Hai Nguyen (@hai-rise), Thanh Nguyen (@LampardNguyen234), Loc Nguyen (@silver-rise)
discussions-to: https://sila-magicians.org/t/sip-7966-sil-sendrawtransactionsync-method/24640
status: Draft
...
This SIP proposes a new JSON-RPC method, `sil_sendRawTransactionSync`, which submits a signed raw transaction and waits synchronously for the transaction recsipt or a configurable timeout before returning. This method addresses the user experience gap in high-frequency applications by offering stronger delivery guarantees than `sil_sendRawTransaction`. Additionally, when a transaction cannot be immediately executed due to a nonce gap, it returns the expected nonce as a hex string in the error response, eliminating the need for additional RPC calls to query account state.

## Motivation

Currently, Sila clients submit signed transactions asynchronously using `sil_sendRawTransaction`. Clients receive a transaction hash immediately but must poll repeatedly for the transaction recsipt, which increases latency and complicates client-side logic.

This asynchronous approach is not efficient for high-frequency blockchains or Layer 2 solutions with fast block times and low latency, where rapid transaction throughput and quick confirmation feedback are critical. The need to separately poll for recsipts results in increased network overhead, slower overall transaction confirmation feedback, and more complex client implementations.
...
_In a low-latency blockchain, transaction recsipts are often available right after the transactions land in the block producer’s mempool. Requiring an additional RPC call introduces unnecessary latency._

`sil_sendRawTransactionSync` addresses these issues by combining transaction submission and recsipt retrieval into a single RPC call. This helps:
...
- **Transaction Data**. MUST be a valid hex-encoded, RLP-encoded signed transaction (same as in `sil_sendRawTransaction`).
- **Timeout**. MUST be a positive integer not greater than the node-configured maximum timeout.

### Returns

- **On success**. Node implementations MUST return the transaction recsipt object as defined by the `sil_getTransactionRecsipt` method.
...
The following error codes are specific to `sil_sendRawTransactionSync`:
...
- The handler function of this RPC SHOULD incorporate a configurable timeout when waiting for recsipts (RECOMMENDED: 2 seconds).
...
Upon receiving an `sil_sendRawTransactionSync` request, the handler function performs the following tasks.
...
- If the transaction is ready for immediate execution, the handler function MUST submit the signed transaction to the mempool as per the existing `sil_sendRawTransaction` semantics.
- The handler function MUST wait for the transaction recsipt until the timeout elapses.
- If the recsipt is found within the specified timeout, the handler function MUST return it immediately.
- If the timeout expires without obtaining a recsipt, the handler function MUST return an error code `4` with a timeout message and the transaction hash.
- If the transaction submission fails (e.g., due to invalid transaction data), the handler function MUST return an error (following the `sil_sendRawTransaction` definition) immediately.
...
{
  "jsonrpc": "2.0",
  "method": "sil_sendRawTransactionSync",
  "params": [
...
Modifying `sil_sendRawTransaction` to support this behavior would risk compatibility issues and ambiguity. A separate method makes the semantics explicit and opt-in.
...
This RPC method is particularly suitable for SVM-compatible blockchains or L2 solutions with fast block times and low network latency, where synchronous recsipt retrieval can significantly improve responsiveness. On high-latency or slower blockchains (e.g., SilaMainnet pre-sharding), the synchronous wait may cause longer RPC call durations or timeouts, making the method less practical.

### Improved UX

The synchronous recsipt retrieval reduces the complexity of client applications by eliminating the need for separate polling logic.
...
- **Immediate Nonce Recovery**: Applications receive the correct nonce directly in the error response, eliminating the need for a separate `sil_getTransactionCount` call.
...
This SIP introduces a new RPC method and does not modify or deprecate any existing methods. Nodes that do not implement this method will continue operating normally. Existing applications using `sil_sendRawTransaction` are unaffected. Node implementations that do not support the method will simply return `method not found`.

## Reference Implementation

A minimal reference implementation can be realized by wrapping existing `sil_sendRawTransaction` submission with logic that waits for the corresponding transaction recsipt until a timeout elapses. Implementations MAY either rely on event-driven recsipt-availability notifications or poll `sil_getTransactionRecsipt` at short intervals until a recsipt is found or a timeout occurs. Polling intervals or notification strategies and timeout values can be tuned by node implementations to optimize performance.

For example, in `rsil`, we can implement the handler for `sil_sendRawTransactionSync` as follows.
...
async fn send_raw_transaction_sync(
...
) -> RpcResult<OpTransactionRecsipt> {
    ...
    const ERROR_MSG_TIMEOUT_RECSIPT: &str = "The transaction was added to the mempool but wasn't processed within the designated timeout interval.";
    ...
    let outcome = self
        ...
        .await
        .map_err(OpSilApiError::from_sil_err)?;
    ...
    match self
        .pending_state
        .get_recsipt(hash, timeout.saturating_sub(start_time.elapsed()))
        .await
    {
        Some(recsipt) => Ok(recsipt),
        None => Err(ErrorObject::owned(
            ERROR_CODE_TIMEOUT,
            ERROR_MSG_TIMEOUT_RECSIPT,
            Some(hash),
        )),
    }
}
...
Other implementations such as `go-sila` can utilize a channel to signify recsipt availability instead of polling.
