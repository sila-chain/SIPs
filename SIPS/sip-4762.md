...
discussions-to: https://sila-magicians.org/t/sip-4762-statelessness-gas-cost-changes/8714
status: Draft
 ...
 * At each step of SVM execution, if and only if `PC < len(code)`, chunk `PC // CHUNK_SIZE` (where `PC` is the current program counter) of the callee is accessed. In particular, note the following corner cases:
 ...
 * If the current step of SVM execution is a `PUSH{n}`, all chunks `(PC // CHUNK_SIZE) <= chunk_index <= ((PC + n) // CHUNK_SIZE)` of the callee are accessed.
