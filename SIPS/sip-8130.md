...
discussions-to: https://sila-magicians.org/t/sip-8130-account-abstraction-by-account-configurations/25952
status: Draft
...
This proposal introduces a new [SIP-2718](./sip-2718.md) transaction type and an onchain Account Configuration system that tosilaer provide account abstraction: custom authentication, call batching, and gas sponsorship. Accounts register actors with onchain authenticator contracts. Transactions declare which authenticator to use, enabling nodes to filter transactions without executing arbitrary wallet code. No SVM changes are required. The contract infrastructure is designed to be shared across chains as a common base layer for account management.

## Motivation

Account abstraction proposals that delegate validation to wallet code force nodes to simulate arbitrary SVM before accepting a transaction. This requires full state access, tracing infrastructure, and reputation systems to bound the cost of invalid submissions.
...
The 8130 transaction type is not required for the accounts to work, and they are fully portable to other SVM-based chains. On chains that don't support the 8130 transaction type, they must use an alternative transport mechanism such as [SRC-4337](./sip-4337.md). See [Portability](#portability).
...
- Canonical authenticators are **enshrined on the validation path** with a constant validation cost (their fixed `sender_auth_cost` / `payer_auth_cost` component values); no SVM runs to authenticate a canonical actor, and the enshrined set changes only through a hard fork.
...
**L2 profile (configurable schedule, canonical-only path).** For an L2 or any high-throughput SVM that wants to track a local cost model, move faster than L1 governance, and keep the transaction path to fixed-cost validation. Under this profile a chain MAY:

- adopt a different [intrinsic-gas schedule](#intrinsic-gas) (for example to mirror a future SVM repricing), provided it is fixed under that chain's consensus and identical across its nodes;
...
Canonical authenticators are **enshrined on the validation path** with a constant validation cost under both [adoption profiles](#adoption-profiles) (their fixed `sender_auth_cost` / `payer_auth_cost` component values); no SVM is executed to authenticate them, and they are accepted on the 8130 transaction path on every compliant chain. When a canonical authenticator is enshrined, its execution MUST produce results identical to the corresponding authenticator contract.

Whether an authenticator **outside the canonical set** is accepted on the 8130 transaction path is the chain's acceptance policy (see [Adoption Profiles](#adoption-profiles)): a **permissive** chain (typically L1) authenticates it with a single `STATICCALL`, metered as ordinary SVM execution and bounded by `MAX_AUTHENTICATION_GAS`; a **canonical-only** chain (typically a high-throughput L2) does not accept it on the transaction path at all.

Any contract implementing `IAuthenticator` can be permissionlessly deployed and registered as an actor's authenticator on any chain. Registration always makes the authenticator usable within SVM execution (for example, authenticating an actor against an arbitrary `IAuthenticator` via a config change call, enabling wallet-defined recovery methods); it becomes usable on the 8130 transaction path only on chains whose acceptance policy is permissive. 
...
Actors are identified by their `actorId`, a 32-byte identifier derived by the authenticator from public key material. Each authenticator defines its own actorId derivation algorithm (see [Canonical Authenticator Set](#canonical-authenticator-set)). Actors can be modified via calls within SVM execution by calling the authenticated change function ([`applySignedAccountChanges`](#account-config-change-paths)).
...
To enable uncoordinated session-key, subscription, and JIT actor additions there is an epoch system. The local change channel carries an **epoch** (`local_epoch`) alongside its sequence counter (`local_sequence`); tosilaer they form the signed local word `local_epoch(high 32) || local_sequence(low 32)` (see [Config Change Authorization](#config-change-authorization)). The epoch is a blunt cancellation control for the account's own outstanding **local** signatures, distinct from actor expiry:
...
Nonce state is managed by a precompile at `NONCE_MANAGER_ADDRESS`. The protocol reads and increments nonce slots directly during AA transaction processing; the precompile exposes a read-only `getNonce()` interface to the SVM.
...
The maximum validity window (`valid_before − now`) accepted for nonce-free transactions is likewise an explicit chain parameter (`NONCE_FREE_EXPIRY_WINDOW`), not a per-node choice, and MUST be sized tosilaer with `REPLAY_BUFFER_CAPACITY` so `peak accepted nonce-free throughput × NONCE_FREE_EXPIRY_WINDOW` stays within capacity. Replay protection is handled by the **replay identifier** defined below.
...
**Full-SVM metering model (normative).** The reference cost of an 8130 transaction is what its work costs when metered as ordinary SVM execution, over four phases:
...
3. **Execution.** Dispatch the transaction's `calls`, metered as ordinary SVM execution.
...
This model is the normative reference for **results**: whatever pricing a chain uses, a transaction MUST be accepted, authenticated, and executed to the same outcome and state changes that phases 1–3 produce under ordinary SVM. A chain MAY implement phases 1 and 2 (account-change application and authentication) as **native** code where applicable (for the enshrined canonical authenticator set and the Keystore reads it depends on), running no SVM for that work and instead charging a fixed gas cost per authenticator and per account-change operation (see [Portability](#portability)). The native path MUST match the Full-SVM model in results, but it need **not** match it in gas: its fixed schedule is its own consensus-fixed pricing, not a re-derivation of what SVM metering would charge. Whether phases 1–2 are metered pure-SVM or priced by a native fixed schedule is decided per chain (see [Adoption Profiles](#adoption-profiles)); phase 3 is always ordinary SVM execution.

The per-component values below are that native fixed schedule for phases 1–2. They approximate the SVM access and data-availability costs ([SIP-2929](./sip-2929.md) / [SIP-2028](./sip-2028.md)) at the time of writing, but a native chain charges them as flat costs rather than metering the underlying SVM. Their normative status follows the chain's [adoption profile](#adoption-profiles):
...
- **L2 profile:** the values below are a **recommended default**. A chain MAY reprice them (for example to track a future SVM repricing or a local cost model), provided the schedule is fixed under that chain's consensus and identical across its nodes.
...
- **`applySignedAccountChanges` (SVM)**: applied during SVM execution on any chain, including non-8130 chains and SRC-4337 deployments.
...
|-----------|-------------|-----------------|
| **Keystore contract** | Protocol reads storage directly for validation; SVM interface available | Standard contract (SRC-4337 compatible factory) |
...
The `DEPLOYMENT_HEADER(n)` is a 14-byte SVM loader that copies trailing code into memory and returns it. The header encodes code length `n` into its `PUSH2` instructions:
...
Account creation, import, signed actor changes, and locking are ordinary SVM entry points on the Keystore contract, so external factories can compose them to mint accounts into a desired end state. This is always possible in SVM but is **not** available on the 8130 transaction path.
...
interface IKeystore {
    ...
    // Protocol-injected receipt log for successful SIP-8130 delegation updates (not emitted in SVM).
