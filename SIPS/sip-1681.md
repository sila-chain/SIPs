...
discussions-to: https://sila-magicians.org/t/temporal-replay-protection/2355
status: Stagnant
...
This SIP is very similar to https://github.com/sila/SIPs/pull/599 by Nick Johnson and Konrad Feldmeier, the main difference
...
- If any form of dust-account clearing is introduced, e.g. (https://github.com/sila/SIPs/issues/168), it will be necessary
to introduce a replay protection, such as https://github.com/sila/SIPs/issues/169 . Having temporal replay protection removes the need
...
Why use walltime instead of block numbers, as proposed in https://github.com/sila/SIPs/pull/599 ?
...
The most notable security impact is that pre-signed transactions stored on paper backups, will become invalid as of block `Y`. There are a couple of cases where this might be used
   - Pregenerated onetime 'bootstrap' transactions, e.g. to onboard a user into Sila. Instead of giving a user a giftcard with actual siler on it, someone may instead give the person a one-time pregenerated transaction that will only send those siler to the card once the
user actively wants to start using it.
   - If a user has an offline paper-wallet, he may have pregenerated transactions to send value to e.g. an exchange. This is sometimes done to be able to send siler to an exchange without having to go through all the hoops of bringing the paper wallet back to 'life'.
