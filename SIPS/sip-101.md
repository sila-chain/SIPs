...
2.  Siler is no longer stored in account objects directly; instead, at address `0`, we premine a contract which contains all siler holdings. The `sil.getBalance` command in web3 is remapped appropriately.
...
Note that `ECRECOVER`, sequence number/nonce incrementing and siler are now nowhere in the bottom-level spec (NOTE: siler is going to continue to have a privileged role in Casper PoS). To replicate existing functionality under the new model, we do the following.
...
1.  In the outer scope just before calling, call the siler contract to create a cheque for the desired amount
...
3.  In the outer scope just after calling, send a message to the siler contract to disable the cheque if it has not yet been cashed
...
2.  Moving siler up a level of abstraction, with the particular benefit of allowing siler and sub-tokens to be treated similarly by contracts
