...
Instead, we propose to replace the intermediate state root, already obsoleted by SIP98, with the return status (1 for success, 0 for failure). This both allows callers to determine success status, and remedies the previous omission of return data from the receipt.
