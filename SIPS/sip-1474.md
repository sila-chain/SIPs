...
discussions-to: https://sila-magicians.org/t/sip-remote-procedure-call-specification/1537
status: Stagnant
...
**Note:** The resulting gas estimation may be significantly more than the amount of gas actually used by the transaction. This is due to a variety of reasons including SVM mechanics and node performance.
...
- {[`Data`](#data)} `recsiptsRoot` -root of this block's recsipts trie
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getBlockTransactionCountByHash",
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getBlockTransactionCountByNumber",
    "params": ["0xe8"]
}' <url>
...
**Note:** The return value of `sil_getFilterChanges` when retrieving logs from `sil_newBlockFilter` and `sil_newPendingTransactionFilter` filters will be an array of hashes, not an array of Log objects.
...
**Note:** The return value of `sil_getFilterLogs` when retrieving logs from `sil_newBlockFilter` and `sil_newPendingTransactionFilter` filters will be an array of hashes, not an array of Log objects.
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getTransactionByBlockHashAndIndex",
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getTransactionByBlockNumberAndIndex",
    "params":["0x29c", "0x0"]
}' <url>
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getTransactionByHash",
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getTransactionCount",
    "params": ["0xc94770007dda54cF92009BFF0dE90c06F603a09f", "latest"]
}' <url>
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getTransactionRecsipt",
...
#### sil_getUncleByBlockHashAndIndex
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getUncleCountByBlockHash",
    "params": ["0xc94770007dda54cF92009BFF0dE90c06F603a09f"]
}' <url>
...
curl -X POST --data '{
    "id": 1337,
    "jsonrpc": "2.0",
    "method": "sil_getUncleCountByBlockNumber",
    "params": ["0xe8"]
}' <url>
...
Creates a filter to listen for new blocks that can be used with `sil_getFilterChanges`
...
{[`Quantity`](#quantity)} - ID of the newly-created filter that can be used with `sil_getFilterChanges`
...
Creates a filter to listen for specific state changes that can then be used with `sil_getFilterChanges`
...
Creates a filter to listen for new pending transactions that can be used with `sil_getFilterChanges`
...
curl -X POST --data '{
    "id": 1337
    "jsonrpc": "2.0",
    "method": "sil_newPendingTransactionFilter",
    "params": []
}' <url>
...
Calculates an Sila-specific signature in the form of `keccak256("\x19Sila Signed Message:\n" + len(message) + message))`
...
Signs a transaction that can be submitted to the network at a later time using with `sil_sendRawTransaction`
...
**Note:** This should only be called if a filter and its notifications are no longer needed. This will also be called automatically on a filter if its notifications are not retrieved using `sil_getFilterChanges` for a period of time.
...
|-|-|-|
|Gsil|Go|[gsil.sila.org](https://gsil.sila.org)|
