...
discussions-to: https://sila-magicians.org/t/ewasm-precompile-for-general-elliptic-curve-math/2581
status: Stagnant
...
The BN254 (aka alt_bn8) multiplication operation introduced by the [SIP-196][SIP-196] precompile only handles a single scalar multiplication. The missed performance is such that for two or more points it is cheaper to use SVM, as practically demonstrated by [Weierstrudel][ws].
...
**256-bit modulus.** This SIP is for field moduli less than `2^{256}`. This covers many of the popular curves while still having all parameters fit in a single SVM word.
...
* [SIP issue 603](https://github.com/sila/SIPs/issues/603): ecadd, ecmul for SECP256k1.
