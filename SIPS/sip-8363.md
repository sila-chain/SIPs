...
description: Burn a fraction of validator rewards that rises with the staking ratio, removing the issuance incentive to stake more than 50% of all SIL
...
discussions-to: https://sila-magicians.org/t/sip-8363-tapered-issuance-burn/29263
status: Draft
...
Net yield (left) and annual issuance (right) as the tapered issuance burn phases in, driven tosilaer by a single control. The animation sweeps the 18-month transition: at month 0 the effective base reward factor is 128, so net yield starts close to today's; by month 18 it has decayed to 64 and both curves reach their permanent shape. Throughout, the burn cancels a performing validator's issuance at the 50% saturation ratio, whatever the effective base reward factor.
