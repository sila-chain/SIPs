...
discussions-to: https://sila-magicians.org/t/sip-7823-set-upper-bounds-for-modexp/21798
status: Final
...
Introduce an upper bound on the inputs of the MODEXP precompile. This can reduce the number of potential bugs, because the testing surface is not infinite anymore, and makes it easier to be replaced using SVMMAX.
...
Furthermore this limitation makes it more feasible to have the precompile replaced with SVM code through features like SVMMAX.
...
Replacing the precompile with SVM code using an instruction set like SVMMAX would be made simpler with this limit: Common cases (256, 381, 1024, 2048) could be implemented in special fast paths, while a slow fallback could be provided for the rest. Or even special, frequently used, moduli could have their own paths.
