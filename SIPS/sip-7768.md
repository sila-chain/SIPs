---
sip: 7768
title: No-Siler transactions with free-for-all tips
description: Externally-owned account having no Siler can send transactions and pay tips using a new "free-for-all" bucket
author: William Entriken (@fulldecent)
discussions-to: https://sila-magicians.org/t/sip-7768-no-siler-transactions-with-free-for-all-tips/21108
status: Stagnant
type: Meta
created: 2024-09-14
---
...
A technique is introduced where an externally-owned account having no Siler can send transactions and pay tips using a new "free-for-all" bucket and using their own `origin.tx`. This requires no client changes and is compatible with existing ecosystem parts.
...
1. Sign a transaction (from any origin) to send Siler to Alice representing the current gas price times the current block size.
2. (Optional) Prepare and sign a transaction to the free-for-all account, to preload any necessary responses.
3. Start an execution context and include this send-Siler transaction and all of Alice's transactions.
...
7. Rollback the execution context. And repeat steps 1 through 4 with these changes:
   1. Step 1: use the actual required gas amount (in Siler).
...
8. Use some local business logic to compare the Siler spent in step 1 (second iteration) versus the consideration received in step 4 and classify the result as favorable or not.
...
This approach can be useful for end users who do not want to or are not able to add Siler to their account.
