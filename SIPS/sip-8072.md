...
The `receipt` field MUST contain the complete transaction receipt object as defined by `sil_getTransactionReceipt`.
...
Monitoring transactions by hash does not introduce new privacy concerns beyond existing `sil_getTransactionReceipt` polling. However, applications should be aware that subscribing to transaction hashes reveals interest in those transactions to the node operator.
