...
**Note on `USABLE_BYTES_PER_FIELD_ELEMENT`**: Field elements are interpreted big-endian (see execution-specs). This SIP sets the first byte (MSB) to zero to ensure that every 32-byte element is a canonical BLS field element (`value < BLS_MODULUS`).
