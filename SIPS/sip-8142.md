...
author: Kevaundray Wedderburn (@kevaundray), Ignacio Hagopian (@jsign), Jihoon Song <jihoon.song@sila.org>, Francesco Risitano (@frisitano), Thomas Thiery (@soispoke) <thomas.thiery@sila.org>, Toni Wahrstätter (@nerolation), Péter Garamvölgyi (@thegaram)
discussions-to: https://sila-magicians.org/t/sip-8142-block-in-blobs-bib/27621
status: Draft
...
zkSVMs allow validators to verify the correctness of an execution payload using a proof, without downloading or executing the payload itself. However, removing the requirement to download the execution payload, also removes the implicit data availability (DA) guarantee; a block producer can publish a valid proof and withhold the execution-payload data since attesters no longer need them for consensus.
...
**Validation with zkSVMs**

With zkSVMs, validators instead:
...
- A zkSVM proof for the block must bind the proven execution payload to those prefixed blob commitments.
...
**Note on `USABLE_BYTES_PER_FIELD_ELEMENT`**: Field elements are interpreted big-endian (see [execution-specs](https://github.com/sila/execution-specs/blob/6137bb32f9b8eda6194096992cb97b28be06957d/src/sila/crypto/kzg.py#L102)). This SIP sets the first byte (MSB) to zero to ensure that every 32-byte element is a canonical BLS field element (`value < BLS_MODULUS`).
...
Specifically, we use the following methods from [polynomial-commitments.md](https://github.com/sila/consensus-specs/blob/46c1199d6b4584ba484dec807f03b8e6211dd725/specs/sila_deneb/polynomial-commitments.md?#introduction):

- [verify_blob_kzg_proof_batch](https://github.com/sila/consensus-specs/blob/46c1199d6b4584ba484dec807f03b8e6211dd725/specs/sila_deneb/polynomial-commitments.md#L578)
- [blob_to_kzg_commitment](https://github.com/sila/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/sila_deneb/polynomial-commitments.md#blob_to_kzg_commitment)
- [compute_blob_kzg_proof](https://github.com/sila/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/sila_deneb/polynomial-commitments.md#compute_blob_kzg_proof)

And the following methods from [polynomial-commitments-sampling.md](https://github.com/sila/consensus-specs/blob/2938e1ad74cea54f1a24508a85704d5bd87837ad/specs/sila_fulu/polynomial-commitments-sampling.md):

- [compute_cells_and_kzg_proofs](https://github.com/sila/consensus-specs/blob/2938e1ad74cea54f1a24508a85704d5bd87837ad/specs/sila_fulu/polynomial-commitments-sampling.md#compute_cells_and_kzg_proofs)

And the following methods from [beacon-chain.md](https://github.com/sila/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/sila_deneb/beacon-chain.md#introduction):

- [kzg_commitment_to_versioned_hash](https://github.com/sila/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/sila_deneb/beacon-chain.md#kzg_commitment_to_versioned_hash)
...
For the zkSVM-optimized variant of `engine_getPayload`, this SIP extends `BlobsBundle` with an additional field:
...
- Used as private inputs to the zkSVM circuit for payload consistency verification via `verify_blob_kzg_proof_batch`
...
- **zkSVM-optimized variant**: Uses polynomial openings via `verify_blob_kzg_proof_batch`. Avoids the multiscalar multiplication (MSM) which is expensive to prove in a zkSVM circuit.
...
##### engine_getPayload - zkSVM-Optimized Variant

For the zkSVM-optimized variant, the builder must additionally compute random point KZG proofs for the payload blobs, which will be used as private inputs in the zkSVM circuit for payload consistency verification.
...
- The builder/prover should extract the first `payload_blob_count` commitments from `all_commitments` (i.e., `all_commitments[:payload_blob_count]`). This corresponds to the `payload_kzg_commitments` parameter in the zkSVM variant of `engine_newPayload`.
...
##### engine_newPayload - zkSVM-Optimized Variant

**Note:** Once zkSVM proofs are required for consensus, `newPayload` will be executed inside a zkSVM to generate a proof, rather than being executed natively by validators. This variant is designed to be cheaper in that context.

This variant replaces the MSM in `blob_to_kzg_commitment` with polynomial opening proofs, which are cheaper to verify inside a zkSVM. The payload, commitments and KZG proofs are private inputs to the zkSVM circuit, while the corresponding versioned hashes (and payload header) are public inputs.
...
### When zkSVM proofs become mandatory, why can't zk-attestors download the full execution payload?
