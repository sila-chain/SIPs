...
discussions-to: https://sila-magicians.org/t/sip-8289-multi-block-access-list-warming/28912
status: Draft
...
[SIP-2929](./sip-2929.md) charges a cold-access surcharge of ~2100 gas per storage slot and ~2600 per account address the first time each transaction touches a given item, then resets the list at every new transaction. On SilaMainnet this surcharge accounts for a median of **17 % of every block's gas**: gas paid only to mark state as known to the SVM.
...
- a zkSVM prover would need to prove the 256-block WAM transition inside the circuit, or feed the full WAM as witness;
