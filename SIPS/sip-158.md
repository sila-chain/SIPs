...
3. Whenever the SVM checks if an account exists, emptiness is treated as equivalent to nonexistence. Particularly, note that this implies that, once this change is enabled, there is no longer a meaningful difference between emptiness and nonexistence from the point of view of SVM execution.
...
- CREATEs (if the code that is ultimately saved is empty and there is no siler remaining in the account when it is saved)
...
1. SIP-158 issue and discussion: https://github.com/sila/SIPs/issues/158
2. SIP-161 issue and discussion: https://github.com/sila/SIPs/issues/161
