...
discussions-to: https://sila-magicians.org/t/sip-8337-validated-evm-code/29013
status: Draft
...
* ***data stack***: the SVM's stack of 256-bit words, at most 1024 deep.
...
* ***execution***: as the Yellow Paper defines it, a sequence of changes
  to the SVM state: one *instruction* at a time, beginning at *PC* 0,
  ending in a halt.
...
The above is a purely *semantic* specification, placing no constraints on the *syntax* of bytecode beyond being an array of opcodes and immediate data.  Subroutines are defined as *subgraphs* of the control-flow graph, not contiguous sequences of bytecode.  The SVM is a simple state machine: each instruction advances it one step, and the *control-flow graph* maps where it can step.  We only promise that *valid* code will not, as it were, jam up the gears of the machine.
...
The reference implementation below is Python: executable and tested.  It validates SVM bytecode against the five constraints defined above, in time and space linear in the size of the code, in circa 195 lines — the `validate()` function itself is circa 150.
