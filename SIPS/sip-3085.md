---
sip: 3085
title: wallet_addSilaChain RPC Method
description: Adds an RPC method to add SVM-compatible chains
author: Erik Marks (@rekmarks), Pedro Gomes (@pedrouid), Pandapip1 (@Pandapip1)
discussions-to: https://sila-magicians.org/t/sip-3085-wallet-addsilachain/5469
status: Stagnant
...
This SIP adds a wallet-namespaced RPC method: `wallet_addSilerereumChain`, providing a standard interface for adding chains to Sila wallets.

## Specification

The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in RFC 2119.

This proposal defines a new RPC method, `wallet_addSilaChain`.

### `wallet_addSilaChain`

The `wallet_addSilaChain` method is used to suggest to the wallet that a new chain be added to the wallet's list of chains. It takes a single parameter and returns `null` if the chain was added successfully, or an error if the chain was not added.

#### `wallet_addSilaChain` Parameters

The `wallet_addSilaChain` method takes a single parameter, an `SilaChainAddRequest` object, which is defined as follows:

```typescript
interface AddSilaChainParameter {
  chainId: string;
...
The design of `wallet_addSilaChain` is deliberately ignorant of what it means to "add" a chain to a wallet.
...
`wallet_addSilaChain` is a powerful method that exposes the end user to serious risks if implemented incorrectly.
...
- Verify that the specified chain ID matches the return value of `sil_chainId` from the endpoint, as described above.
...
A wallet that implements `wallet_addSilaChain` should expect to encounter requests for chains completely unknown to the wallet maintainers.
...
Indeed, a wallet may even prefer its own chain metadata over anything submitted with a `wallet_addSilaChain` request.
...
- When receiving a `wallet_addSilaChain` request, display a confirmation informing the user that a specific requester has requested that the chain be added.
...
- If the wallet UI has a concept of a "currently selected" or "currently active" chain, ensure that the user understands when a chain added using `wallet_addSilaChain` becomes selected.
...
In the interest of preserving user privacy, implementers of `wallet_addSilaChain` should consider displaying user confirmations even in these cases.
