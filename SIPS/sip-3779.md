---
sip: 3779
title: Safer Control Flow for the SVM
description: Ensure an essential level of safety for SVM code.
...
category: Core
created: 2021-08-30
withdrawal-reason: material moved to SIP-2315
---

## Abstract

We define a safe SVM contract as one that cannot encounter an exceptional halting state.  In general, we cannot prove safety for Turing-complete programs. But we can prove a useful subset.
...
We define a _safe_ SVM contract as one that cannot encounter an exceptional halting state.  We validate _safety_ at initialization time to the extent practical.

#### *Exceptional Halting States*

The *execution* of each instruction is defined in the Yellow Paper as a change to the SVM state that preserves the invariants of SVM state.  At runtime, if the execution of an instruction would violate an invariant the SVM is in an exceptional halting state.  The Yellow Paper defined five such states.
1. Insufficient gas
...
*We would like to consider SVM programs valid iff they are safe.*
...
Taken tosilaer, these rules allow for code to be validated by traversing the control-flow graph, in time and space linear in the size of the code, following each edge only once. 
...
These changes affect the semantics of SVM code – the use of `JUMP`, `JUMPI`, and the stack are restricted, such that some *code* that would otherwise run correctly will nonetheless be invalid SVM *code*. 
...
This SIP is intended to ensure an essential level of safety for SVM code deployed on the blockchain.
