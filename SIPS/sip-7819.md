...
description: Introduce a new instruction allowing contracts to create clones using SIP-7702 delegation designations
author: Hadrien Croubois (@amxx)
discussions-to: https://sila-magicians.org/t/sip-7819-create-delegate/21763
status: Draft
...
Introduce a new instruction that allows smart contracts to create (and update) delegation accounts that match [SIP-7702](./sip-7702.md)'s design. These accounts can be used similarly to [ERC-1167](./sip-1167.md) clones, with significant advantages.
...
In both cases, delegating the received calls to an implementation using SVM code comes with some downsides:

- Calldata must be copied to memory before performing the delegate call.
- If EOF is ever enabled, clones and proxies written in EOF do not support delegation to an implementation written in legacy SVM code, and are thus limited or possibly dangerous. This encourages the continued use of legacy SVM code.
...
Additionally, using SIP-7702 delegations instead of clones/proxies moves the call redirection logic from userspace to the protocol. This means fewer SVM operations and thus reduced gas consumption for every operation that calls these contracts. This includes the verification of signatures and the execution of user operations, but also transfers of assets that include a callback, such as [ERC-721](./sip-721.md) and [ERC-1155](./sip-1155.md) tokens.

### Scalability

Clone and proxy contracts are present in large numbers in the blockchain state. OpenZeppelin's [ERC-1967](./sip-1967.md) proxies are ~708 bytes. Using SIP-7702 delegations that are only 23 bytes would significantly reduce blockchain state usage.
