---
sip: 3326
title: Wallet Switch Sila Chain RPC Method (`wallet_switchSilaChain`)
author: Erik Marks (@rekmarks)
discussions-to: https://sila-magicians.org/t/sip-3326-wallet-switchsilachain
status: Stagnant
...
The `wallet_switchSilaChain` RPC method allows Sila applications ("dapps") to request that the wallet switches its active Sila chain, if the wallet has a concept thereof.
...
`wallet_switchSilaChain` enables dapps to request that the wallet switches its active chain to whichever one is required by the dapp.
...
`wallet_switchSilaChain` accepts a single object parameter, specified by the following TypeScript interface:

```typescript
interface SwitchSilaChainParameter {
  chainId: string;
}
```
...
- `chainId`
  - **MUST** specify the integer ID of the chain as a hexadecimal string, per the [`sil_chainId`](./sip-695.md) Sila RPC method.
...
The purpose `wallet_switchSilaChain` is to provide dapps with a way of requesting to switch the wallet's active chain, which they would otherwise have to ask the user to do manually.
...
For related work, see [SIP-3085: `wallet_addSilaChain`](./sip-3085.md) and [SIP-2015: `wallet_updateSilaChain`](./sip-2015.md).
`wallet_switchSilaChain` intentionally forgoes the chain metadata parameters included in those SIPs, since it is purely concerned with switching the active chain, regardless of RPC endpoints or any other metadata associated therewith.
...
- Display a confirmation whenever a `wallet_switchSilaChain` is received, clearly identifying the requester and the chain that will be switched to.
