...
description: Adds SVM opcodes for efficient single-byte memory and calldata loads.
author: Helkomine (@Helkomine)
discussions-to: https://sila-magicians.org/t/sip-8120-mload8-and-calldataload8-opcodes/27396
status: Draft
...
This SIP introduces new SVM opcodes that allow loading a single byte from memory or calldata in a single operation, reducing gas cost and bytecode size compared to existing patterns based on `MLOAD (0x51)` or `CALLDATALOAD (0x35)` followed by bit shifting.
...
The base gas cost matches `MLOAD`, `MSTORE8`, and `CALLDATALOAD`, ensuring consistency with existing SVM pricing.
...
In both cases, execution halts and the current call frame is reverted, consistent with existing SVM behavior.
