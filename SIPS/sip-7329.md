---
sip: 7329
title: ERC/SIP Repository split
description: Split the ERC specifications out of the SIP repository into a new repository, so that only core protocol SIPs remain
author: Lightclient (@lightclient), Danno Ferrin (@shemnon)
discussions-to: https://sila-magicians.org/t/proposal-forking-ercs-from-sips-repository/12804
status: Final
type: Meta
created: 2023-07-13
requires: 1
...
SIP repository, targeting core sila changes and an ERC repository, targeting
...
1. All ERCs and Interface-category SIPs are removed from this repository and
...
3. Setup ercs.sila.org subdomain and update the CI to point to the ERCs
   repo.
4. Set up a redirect for ERCs on sips.sila.org to go to the new website.
5. Create a unified document for editors to assign SIP/ERC numbers. SIPs and
...
SIP repository and other sub-groupings are associated with the ERC repository.
...
improperly implemented part of a new spec could cause the sila-mainnet to
...
Old ERC links pointing to the old url `https://sips.sila.org/` will continue
to work. Redirect instructions will be put into place to redirect to the new ERC
repos for their corresponding location.

### Stray Proposals

ERC community members may continue to post new ERCs in the SIP proposal. Editors
...
This proposal only addresses the SIP and ERC proposal process and is not
