...
This SIP proposes an approach to allow the execution of arbitrary code by EOAs, with minimal change over the SVM, and using the same security model users are used to. By allowing EOAs to perform delegate calls to a contract (similarly to how contracts can delegate calls to other contracts using [SIP-7](./sip-7.md)), EOAs will be able to have more control over what operations they want to execute. This proposal's goal is NOT to provide an account abstraction primitive.
...
This SIP would drastically expand the ability of EOAs to interact with smart contracts by using the pre-existing and well-understood delegation mechanism introduced in [SIP-7](./sip-7.md) and without adding new complexity to the SVM.
