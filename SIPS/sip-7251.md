...
description: Allow validators to have larger effective balances, while maintaining the 32 SIL lower bound.
...
discussions-to: https://sila-magicians.org/t/sip-7251-increase-the-max-effective-balance/15982
status: Final
...
7. Modify `initialize_beacon_state_from_sil1` to use `MIN_ACTIVATION_BALANCE`.
...
Full consensus layer specification can be found in [`./electra/beacon-chain.md`](https://github.com/sila/consensus-specs/blob/834e40604ae4411e565bd6540da50b008b2496dc/specs/electra/beacon-chain.md)
...
Note: the system contract uses the SVM `CALLER` operation (Solidity: `msg.sender`) to get the address used in the consolidation request, i.e. the address that calls the system contract must match the 0x01 withdrawal credential recorded in the beacon state.
