...
discussions-to: https://github.com/sila/SIPs/issues/2697
status: Final
...
interface SilaProvider {
	request(args: RequestArguments): Promise<unknown>
}
```
The Provider **MUST** implement a `request` method on the exposed `SilaProvider` object.  The `request` method **MUST** be callable with a single parameter which contains the arguments for the request as defined in the TypeScript `interface` above.
...
```
Then the matching `SilaProvider.request` call would be:
```typescript
declare const provider: SilaProvider
...
```
Then the matching `SilaProvider.request` response would be:
```typescript
{ color: 'red', value: 5 }
```
