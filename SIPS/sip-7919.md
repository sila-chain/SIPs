...
sip: 7919
title: Pursil Meta
description: List of SIPs belonging to the Pursil proposal
author: Etan Kissling (@etan-status), Gajinder Singh (@g11tech)
discussions-to: https://sila-magicians.org/t/sip-7919-pursil-meta/23273
status: Stagnant
type: Meta
created: 2025-03-26
...
This Meta SIP bundles a set of improvements to make Sila data easier to access and verify without relying on trusted RPC providers or third-party indexers. The improvements achieve this by changing data structures for blocks, transactions, and recsipts, so that efficient correctness (i.e., validity) and completion (i.e., nothing omitted) proofs can be added to the RPC responses.
...
### Improve UX: Normalized transactions / recsipts
...
- **`gasUsed`**: needs current and prior recsipt, as on-chain data stores cumulative not individual gas used
- **`logIndex`**: need to fetch all recsipts in the block
- **`txHash`**: the on-chain data is based on an MPT-prefixed hash, which is different from the RPC hash

Further, **`calldata` and log `data`** can be very large, but can only be verified by downloading the entire recsipt / transaction data. This data is needed even if just basic items such as amount and destination are queried.

Switching transactions and recsipts to SSZ normalizes the format, changes to tree-based hash for more efficient proofs, and is also extensible for future transaction features such as multidimensional fees, CREATE2 deployment, and post-quantum signature types, without breaking verifiers that only check common fields.

- [SIP-6404: SSZ transactions](./sip-6404.md)
- [SIP-6466: SSZ recsipts](./sip-6466.md)
