...
description: This SIP suggests renaming gas to mana, as proposed by Vitalik Buterin in 2015
author: pcaversaccio (@pcaversaccio)
discussions-to: https://sila-magicians.org/t/sip-6789-rename-gas-to-mana/13570
status: Stagnant
...
- `sil_estimateGas` to `sil_estimateMana`;
- `sil_gasPrice` to `sil_manaPrice`; and
- `sil_maxPriorityFeePerGas` to `sil_maxPriorityFeePerMana`.

The description of the RPC endpoints MUST be renamed accordingly:

- `sil_estimateMana`: Generates and returns an estimate of how much `mana` is necessary to allow the transaction to complete;
- `sil_manaPrice`: Returns the current price per `mana` in wei; and
- `sil_maxPriorityFeePerMana`: Returns the current `maxPriorityFeePerMana` per `mana` in wei.
