...
We can also incentivize full nodes to propagate transactions and to store transactions or state, in addition to verifying them. While these first two incentivizations are outside the scope of this SIP, there are proposals [here](Sila Research and [here](Sila Research respectively.
...
Regarding rewards for full nodes, in the [draft phase 1 sharding spec](Sila Research proposers acting as full nodes have rewards for proposing blobs (without execution) or later in phase 3 transactions (with execution) to be included into collations/blocks. So that would help. However, full nodes that do not act as proposers and just verify transactions, or [full state nodes](Sila Research are still not incentivized.
