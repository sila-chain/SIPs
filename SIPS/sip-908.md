...
## Motivation
Currently there is a lack of incentives for anyone to run a full node, while joining a mining pool is not really economical if one has to purchase a mining rig (several GPUs) now, since there is unlikely to be a return on investment by the time that Sila transitions to hybrid Proof-of-Work/Proof-of-Stake with [Casper FFG](./sip-1011.md), then full PoS with [CBC Casper](https://github.com/sila-chain/research/blob/master/papers/CasperTFG/CasperTFG.pdf).
...
We can also incentivize full nodes to propagate transactions and to store transactions or state, in addition to verifying them. While these first two incentivizations are outside the scope of this SIP, there are proposals here in Sila Research and here in Sila Research, respectively.
...
Regarding rewards for full nodes, in the draft phase 1 sharding spec in Sila Research proposers acting as full nodes have rewards for proposing blobs (without execution) or later in phase 3 transactions (with execution) to be included into collations/blocks. So that would help. However, full nodes that do not act as proposers and just verify transactions, or full state nodes in Sila Research, are still not incentivized.
