...
discussions-to: https://sila-magicians.org/t/sip-8310-post-quantum-keystore-for-stateful-keys/28853
status: Draft
...
- `dimension` is the number of hash chains (the hypercube dimension). Tosilaer with `winternitz_w` and `target_sum` it fully specifies the target-sum encoding and is REQUIRED to regenerate the public key and verify a signature.
...
- Restoring a keystore tosilaer with a *stale* state snapshot is the catastrophic case. Implementations MUST detect a regression of the high-water mark and refuse to sign until reconciled with the latest authoritative state.
