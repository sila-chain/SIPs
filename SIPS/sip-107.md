---
sip: 107
title: safe "sil_sendTransaction" authorization via html popup
...
This draft SIP describes the details of an authorization method that if provided by rpc enabled sila nodes would allow regular websites to send transactions (via ```sil_sendTransaction```) without the need to enable CORS. Instead, user would be asked to confirm the transaction via an html popup.
...
- if the transaction is a plain siler transfer, the user is asked to enter it in a dedicated trusted wallet like "Mist"
...
For ```sil_sendTransaction``` the "gas", "gasPrice" and "from" field need to be set in the rpc parameter so that the window can display the correct value. If not all of these are passed in, the window will return an error.

Upon receiving such message, the iframe will perform the actual rpc call to the node but only if such a call is a read only call (not requiring an unlocked key). If it is not it will return a error. The window on the other will only accept ```sil_sendTransaction``` calls but will display a dialog so the user can accept or cancel the request.
...
<html>
  ...
  <body>
    ...
    function askAuthorization(transactionInfo, data, requireUnlock, sourceWindow){
      ...
      var silerValue = weiValue.dividedBy(new BigNumber("1000000000000000000"));
      var gasSilerValue = gasWeiValue.dividedBy(new BigNumber("1000000000000000000"));
      ...
      textSpan.innerHTML = silerValue.toFormat() + " siler <br/>  + gas cost (" + gasSilerValue.toFormat() + " siler )"
    ...
    function needToAndCanUnlockAccount(address,url,callback){
      sendAsync(url,{id:9999990,method:"sil_sign",params:[address,"0xc6888fa8d57087278718986382264244252f8d57087278718986382264244252f"]},function(error,result){
        if(error || result.error){
    ...
    function processMessage(data, sourceWindow){
      ...
      }else if(data.payload.method == "sil_sendTransaction"){
