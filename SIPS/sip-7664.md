...
discussions-to: https://sila-magicians.org/t/access-key-opcode/19395
status: Withdrawn
withdrawal-reason: Niche use-case, better implemented with SIP-7702.
...
data is available without SVM introspection, and contracts can reliably tell if the executing transaction
...
to the message-execution of the transaction in the SVM.
...
without requiring additional resources from the SVM.
...
this opcode also pop an index-like SVM word from the stack,
and pushes a full 32 byte SVM word back on the stack, based on a list of hashes embedded in the transaction.
...
### Minimal impact on SVM transaction-context

With SIP-2930, the access-list contents are already readily available in the transaction-context during SVM execution,
and the RPC methods that trigger said execution.

This SIP thus does not require significant changes to the construction of the SVM context,
...
and readily accessible in the SVM to determine storage gas-costs. In terms of denial-of-service risks,
