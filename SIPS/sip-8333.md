...
discussions-to: https://sila-magicians.org/t/sip-8333-align-checkpoint-with-epoch-boundary-block/29003
status: Draft
...
The current anchoring is also a long-standing off-by-one rather than a deliberate design decision (`sila-chain/consensus-specs` issues 2174 and 652), and it makes finalization semantics misleading. "Epoch `N` finalized" today means only that the first block of epoch `N` is finalized; the rest of the epoch's blocks can still be reorged. Under this proposal a checkpoint for epoch `N` names the chain through the end of epoch `N - 1`, so finalizing it finalizes whole epochs, nothing more and nothing less. Explorers and tooling that report finality per epoch become accurate.

## Specification

The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in [RFC 2119](https://www.rfc-editor.org/rfc/rfc2119) and [RFC 8174](https://www.rfc-editor.org/rfc/rfc8174).

Functions, constants, and types used below without definition, such as `get_block_root` and `get_checkpoint_block`, are defined in the `sila-chain/consensus-specs` repository.
