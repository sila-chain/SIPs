...
discussions-to: https://sila-magicians.org/t/sip-2583-penalties-for-trie-misses/4190
status: Stagnant
...
- Implement flattened account databases. This is also difficult, both because of reorgs and also because it needs to be an _additional_ data structure aside from the `trie` -- we need the `trie` for consensus. So it's an extra data structure of around `15G` that needs to be kept in check. This is currently being pursued by the Gsil-team.
...
Whether the rules of this SIP is to be applied for regular siler-sends in `transactions` is TBD. See the 'Backwards Compatibility'-section for some more discussion on that topic.
...
[ERC223 Token Standard](https://github.com/sila/SIPs/issues/223) is, at the time of writing, marked as 'Draft', but is deployed and in use on sila-mainnet today.
...
The contract [`Dentacoin`](https://silerscan.io/address/0x08d32b0da63e2c3bcf8019c9c5d849d7a9d791e6#code) would be affected.
...
The contract checks `_to.balance >= minBalanceForAccounts`, and if the `balance` is too low, some `DCN` is converted to `siler` and sent to the `_to`. This is a mechanism to ease on-boarding, whereby a new user who has received some `DCN` can immediately create a transaction.

Before this SIP:

- When sending `DCN` to a non-existing address, the additional `gas` expenditure would be:
  - `9000` for an siler-transfer
