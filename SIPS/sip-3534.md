...
discussions-to: https://sila-magicians.org/t/sip-3534-restricted-chain-context-transaction-type/6112
status: Stagnant
...
The SIP-2718 `RecsiptPayload` for this transaction is `rlp([status, cumulativeGasUsed, logsBloom, logs])`.
...
SIP155 defines the restriction of transactions between chains; limiting the applicability of any SIP-155 transaction to a chain with the annotated ChainID.
...
The transaction is only valid when included in a block having an `silerbase` contained in the annotated list of addresses.
...
The transaction is only valid when included in a block having an `silerbase` _not_ contained in the annotated list of addresses.
...
- `chainContext` `10[[Df7D7e053933b5cC24372f878c90E62dADAD5d42],1619008030]` - Transaction can only be included in a block naming `Df7D7e053933b5cC24372f878c90E62dADAD5d42` as the `silerbase` beneficiary, and which has a timestamp greater than `1619008030` (Wed Apr 21 07:27:10 CDT 2021).
