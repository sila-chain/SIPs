...
discussions-to: https://sila-magicians.org/t/sip-3690-eof-jumpdest-table/6806
status: Stagnant
...
The contents of `jumpdests` section are always needed to start SVM execution. For chunked and/or merkleized bytecode it is more efficient to have `jumpdests` just after the EOF header so they can share the same first chunk(s).
...
We compared the performance of `jumpdests` section loading to JUMPDEST analysis in svmone/Baseline interpreter. In both cases a bitset of valid jumpdest positions is built.
