...
discussions-to: https://sila-magicians.org/t/sip-8250-keyed-nonces-for-frame-transactions/28437
status: Draft
...
Gas deducted as `KEYED_NONCE_FIRST_USE_GAS` is gas used by the frame executing `APPROVE`. When charged, the surcharge is included in the frame's `gas_used` recsipt value, transaction gas accounting, and SIP-8141 unpaid-gas refund calculation.
...
`nonce_keys == [0]` aliases the legacy account nonce so SIP-8141's existing replay behavior is preserved. Account deployment, `CREATE`, and `CREATE2` continue to affect account nonces by ordinary SVM rules. Only payment-approval nonce-bumping is replaced.

`nonce_keys` are `uint256` values because privacy protocols often derive keys from 32-byte nullifiers, commitments, or hash outputs. [ERC-4337](./sip-4337.md) uses the same key/sequence model but has only one 32-byte nonce field, so it packs a 24-byte key and an 8-byte sequence into that field. This SIP uses explicit fields: 32-byte nonce keys and an 8-byte `nonce_seq`. That keeps ERC-4337's 64-bit sequence width, avoids truncating nullifier-derived labels, and makes ERC-4337-style 24-byte keys a subset of this SIP's key space.
...
`nonce_keys == [0]` preserves SIP-8141 replay behavior. Legacy account objects, non-frame transaction types, and `sil_getTransactionCount` are unchanged.
