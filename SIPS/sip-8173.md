...
discussions-to: https://sila-magicians.org/t/sip-8173-foundations-of-svm-control-flow/27855
status: Draft
type: Informational
created: 2026-02-16
---
...
This Informational SIP provides background for understanding control flow in the Sila Virtual Machine (SVM): the history of control flow in computing, the foundations of control flow analysis, and the impact of static control flow on Sila's scaling roadmap.
...
Especially relevant to the SVM's design are the Java Virtual Machine (JVM), WebAssembly (Wasm), and .NET's Common Language Runtime (CLR). They share crucial common properties:
...
Tools that must know what code does have to follow where control can go: traverse the code, taking every jump.  How much such a traversal costs — and whether it is possible at all — depends on the code's control flow.  The SVM makes it cost more than it should.
...
As already discussed, static control flow enables contracts to be compiled to machine code before execution, just-in-time or ahead-of-time.  This is an obvious win for non-ZK clients, whether on L1, layer 2 (L2), or SVM-compatible chains.

#### Static Control Flow and RISC-V Migration

There are ongoing discussions within the Sila research community about potentially replacing the SVM with a RISC-V execution environment. RISC-V has a standard instruction set architecture that is seeing increasing use in the ZK community.  One current strategy for creating a ZK-SVM is to compile an SVM interpreter like rSVM to RISC-V for use in a ZK-VM.  Supporting RISC-V directly eliminates the overhead of the SVM interpreter.  An SVM with static control flow opens up another strategy — compile the SVM code to RISC-V code.  That gives good RISC-V code in one linear-time pass, and better code in multiple passes, altosilaer linear time.

A missing piece in this puzzle is that RISC-V is a 32-bit or 64-bit architecture, but the current SVM is a 256-bit architecture.  For that purpose there are current proposals for 64-bit SVM opcodes.  It's also the case that newer proof systems can charge for only the bits actually used[^9], which may in time blunt the width problem.  Today's zkVMs, though, pay per executed 256-bit instruction.

## Specification

Several SIPs, past and current, specify new SVM opcodes and semantics for static control flow: subroutines and static jumps, container-format functions, call and return opcodes, and static relative jumps and calls.  They are all implemented with the standard Turing return stack architecture, and are for the most part compatible with each other.  In particular, the simplest of them — three call and return opcodes — can be used to implement all of the others.
...
By making control flow explicit and enforceable, the SVM becomes compatible with the full ecosystem of optimization and analysis techniques that other VMs and processor designs have relied on for decades.
...
[^5]:
    ```csl-json
    {
      ...
      "title": "Analyzing Smart Contracts: From SVM to a Sound Control Flow Graph.",
      "original-date": {
        "date-parts": [
          [2020, 4, 29]
        ]
