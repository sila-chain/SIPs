...
There are ongoing discussions within the Sila research community about potentially replacing the SVM with a RISC-V execution environment. RISC-V has a standard instruction set architecture that is seeing increasing use in the ZK community.  One current strategy for creating a ZK-EVM is to compile an SVM interpreter like revm to RISC-V for use in a ZK-VM.  Supporting RISC-V directly eliminates the overhead of the SVM interpreter.  An SVM with static control flow opens up another strategy — compile the SVM code to RISC-V code.  That gives good RISC-V code in one linear-time pass, and better code in multiple passes, altogether linear time.
