...
discussions-to: https://sila-magicians.org/t/sip-3416-median-gas-premium/5755
status: Stagnant
...
* Gas prices spikes are mathematically smoothed out. SIP1559 does not seems to really tackle gas premium volatility and UX.
...
* `BASE_FEE`: 16th item in the block header. Represents the amount of attosil burned for every unit of gas a transaction uses.
...
* During transaction execution, for SIP3416 transactions we calculate the cost to the `tx.origin` and the gain to the `block.coinbase` as follows:
