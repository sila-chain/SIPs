...
discussions-to: https://sila-magicians.org/t/sip-draft-transaction-data-opcodes/6017
status: Stagnant
...
This SIP introduces the following three SVM instructions: `ORIGINDATALOAD`, `ORIGINDATASIZE`, and `ORIGINDATACOPY`.
...
As the Sila development scene matures, more ambitious and complex features are introduced into smart contracts more often than not requiring the utilization of complex and at times large data structures. Given the inherent limits of the SVM, however, transporting large data structures in between contracts is a costly task that can at times lead to even futile scenarios whereby the gas consumption of such an operation is impossible to execute within the gas limit bounds as well as without sacrificing a large chunk of SIL to facilitate its gas cost.
...
One can argue that multiple new SVM instructions pollute the SVM instruction address space and could cause issues in assigning sensible instruction codes to future instructions. This particular issue was assessed and a methodology via which the raw RLP encoded transaction may be accessible to the SVM was ideated. This would _future-proof_ the new instruction set as it would be usable for other members of the transaction that may be desired to be accessible on-chain in the future, however, it would also cause a redundancy in the `ORIGIN` opcode.

## Backwards Compatibility

The SIP does not alter or adjust existing functionality provided by the SVM and as such, no known issues exist.
...
An initial concern that may arise from this SIP is the additional contextual data that must be provided at the software level of nodes to the SVM in order for it to be able to access the necessary data via the `ORIGINDATALOAD` and `ORIGINDATACOPY` instructions.
