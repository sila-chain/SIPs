...
discussions-to: https://sila-magicians.org/t/frame-transaction/27617
status: Draft
...
Ultimately, frame transactions realize the original vision of account abstraction: an account simply becomes an address with code. It leverages the SVM to support arbitrary *user-defined* definitions of validation and gas payment.
...
The `signatures` list contains signatures that may be referenced by `VERIFY` frames and by ordinary SVM execution. Every protocol-validated signature in this list must validate successfully before any frame is executed. If any signature is malformed, or any protocol-validated signature is invalid, the whole transaction is invalid.
...
The raw `signature` byte strings of protocol-validated schemes are intentionally not introspectable by SVM code to allow future aggregation schemes. Contracts can inspect only the metadata of protocol-validated signature entries through transaction introspection. The raw `signature` byte strings of `ARBITRARY` entries are introspectable by SVM code through the `SIGDATACOPY` instruction. This means that they cannot be aggregated in the future.
...
The explicit 32-byte zero digest is invalid. This reserves the zero stack value as the SVM-visible representation of the transaction signing hash case.
...
Note: since the signature validation does not happen in SVM execution, the related precompiles `ecrecover` and `P256VERIFY` must not be added to the block-level access list.
...
```python
if len(svm.calldata) != EXPIRY_DATA_LENGTH:
    revert()

expiry_timestamp = int.from_bytes(svm.calldata, "big")
if svm.timestamp > expiry_timestamp:
    revert()

stop()
```

If the check passes, the frame succeeds with no return data and no logs. The frame consumes gas according to normal SVM execution rules.

Clients may omit the explicit SVM execution and directly perform the expiry check above, provided the externally observable result is identical to executing the canonical contract. Note: While this is a valid optimization for Sila sila-mainnet, it could be problematic on non-sila-mainnet situations in case a different contract is used.
...
1. If a frame is part of an atomic batch and it fails, unroll the associated atomic batch.
    ...
    - Logs emitted by frames that executed before the failure are discarded tosilaer with their state changes when the batch is unrolled. Changes to any frame receipt's `gas_used.state` during the batch are reverted, as the batch's state modifications are reverted. Executed frame receipts retain their execution status and execution gas used, with empty logs and zero state gas used.
...
- Execution-gas draws from `gas_left`, under unchanged SVM semantics. `gas_left` is the standard callframe gas pool. The `GAS` instruction returns the current callframe's `gas_left` and never observes `state_gas_left`. The `GAS_CALL_STIPEND` pre-state check of SIP-8037 applies to `gas_left` only.
- State-gas draws from `state_gas_left` only. `state_gas_left` is frame-scoped. SVM call frames at any depth within the frame draw from it directly. It is not forwarded, split, or divided per callframe like execution gas.
...
- When an SVM call frame reverts or halts exceptionally, restore the state-gas journal and the active frame's `state_gas_left` to the checkpoint covering that call and any associated pre-call state charge.
...
The `APPROVE` instruction exits the current SVM call frame successfully and updates the transaction-scoped approval context based on the `scope` operand. The `offset` and `length` operands designate a memory region that becomes the call frame's return data, following `RETURN` semantics.
...
the standard SVM memory expansion cost.
...
This opcode copies a signature's raw `signature` bytes into the contract's memory. The raw `signature` bytes of protocol-validated schemes are intentionally not accessible from the SVM. `ARBITRARY` signature bytes are validated during SVM execution and therefore may be copied. Its gas cost is calculated exactly as for `CALLDATACOPY`, including the fixed cost of 3, the per-word copy cost, and the standard SVM memory expansion cost.
...
5. if a paymaster frame is present, either a canonical paymaster instance tosilaer with explicit paymaster balance reservation, or a non-canonical paymaster being used by less than `MAX_PENDING_TXS_USING_NON_CANONICAL_PAYMASTER` pending transactions,
...
Although `TIMESTAMP` is generally banned during validation-prefix execution, it is permitted when executing the canonical expiry verifier runtime code at `EXPIRY_VERIFIER`; clients may optimize the frame by performing the same deadline check without explicit SVM execution.
...
Atomic batching allows multiple frames to be grouped into a single all-or-nothing unit. This is useful when a sequence of calls is only meaningful if all succeed tosilaer, such as an approval followed by a swap, or a series of interdependent state changes. Without this feature, a revert in one frame would leave the preceding frames' state changes applied, potentially leaving the account in an undesirable intermediate state.
