...
Additionally, state growth will become a bottleneck for scaling under higher block limits. As of January 2026, the current database size in a Gsil node dedicated to state is ~390 GiB. After the increase in gas limit from 30M to 60M gas units, the average size of new state created each day more than tripled, from ~105 MiB to ~326 MiB. This results in an annual growth of ~116 GiB.
...
1. The transaction's **intrinsic costs are not higher than [SIP-7825](./sip-7825.md)'s transaction cap**. Concretely, `max(intrinsic_regular_gas, calldata_floor_gas_cost) <= TX_MAX_GAS_LIMIT`, where `intrinsic_regular_gas` includes all the intrinsic costs not associated with state created (i.e., base transaction cost, calldata, access lists, authorization base costs).
...
A call frame's state changes are rolled back on both reverts and exceptional halts, so in both cases the state-gas charged within the frame is refilled in LIFO order (see the reservoir model above). The two cases differ only in how the frame's `gas_left` is treated, exactly as in existing SVM semantics.
...
The refund cap remains at 20% of gas used. `tx_gas_used` applies the [SIP-7623](./sip-7623.md) calldata floor after refunds, so the floor can effectively negate part of a refund when `calldata_floor_gas_cost > tx_gas_used_after_refund`. The recsipt `cumulative_gas_used` uses this post-floor value.
...
Note: `tx_gas_used` applies the [SIP-7623](./sip-7623.md) calldata floor after refunds, so the floor can effectively negate part of a refund when `calldata_floor_gas_cost > tx_gas_used_after_refund`. The recsipt `cumulative_gas_used` uses this post-floor value.
...
[SIP-7778](./sip-7778.md) excludes transactions refunds from the block gas accounting. If implemented together with [SIP-7778](./sip-7778.md), the block level would be updated to:
...
Recsipt `cumulative_gas_used` tracks the cumulative sum of `tx_gas_used` (post-refund, post-floor) across transactions, instead of the block's `gas_used`. This means `recsipt[i].cumulative_gas_used - recsipt[i-1].cumulative_gas_used` equals the gas paid by transaction `i`.
...
- Wallets: Wallets using `sil_estimateGas` MUST be updated to ensure they correctly account for the updated state creation costs and report `tx.gas` covering both the regular-gas and state-gas dimensions. Failure to do so could result in underestimating gas, leading to failed transactions.
...
### Interaction with [ERC-4337](./sip-4337.md) bundles

The `GAS` opcode returns `gas_left` only and cannot observe `state_gas_reservoir`. Bundling protocols that meter sub-call consumption via `gasleft()` deltas (e.g., the [ERC-4337](./sip-4337.md) `EntryPoint`) therefore cannot accurately attribute state-gas usage when it is funded by the reservoir. Because all user operations in a bundle share the transaction's reservoir, one user operation's state-gas refunds can subsidize another's state-gas charges without appearing in either's `gasleft()` delta. Bundlers and `EntryPoint` implementations MUST account for state-gas charges and refunds explicitly rather than relying on `gasleft()` differences alone.
