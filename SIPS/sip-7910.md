---
sip: 7910
title: sil_config JSON-RPC Method
description: A JSON-RPC method that describes the configuration of the current and next fork
author: Danno Ferrin (@shemnon)
discussions-to: https://sila-magicians.org/t/sil-config-json-rpc-method/23183
status: Final
...
- Automated pre-fork validation scripts comparing `sil_config` outputs across nodes.
...
A new JSON-RPC API, `sil_config`, is introduced. It takes no parameters and returns the result object specified in the next section.
...
The purpose of this specification is to enable nodes to advertise, prior to a fork, that they have the correct configurations loaded and ready. Past testnet and SilaMainnet forks have revealed clients with incorrect precompile sets, chain IDs, deposit contract addresses, and other configuration errors.
...
Generally, if a configurable variable or constant causes a client to diverge at a fork—whether on SilaMainnet, a testnet, a devnet, or a public rollup—that variable or constant is a candidate for inclusion in the reportable configuration.
...
This SIP does not alter previous behavior. Configurations prior to Cancun are non-standard, and clients sharing pre-Cancun configurations will produce non-standard results.
...
```bash
curl -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"sil_config","id":1}' http://localhost:8545
```
...
- **Exposure Risks**: Incorrect configurations could leak operational details. Operators SHOULD restrict `sil_config` to trusted interfaces (e.g., local access or authenticated endpoints).
- **Dishonest Nodes**: Clients may report false configurations. Peers or monitoring tools MAY cross-check `sil_config` outputs against known fork specifications or other nodes' responses to detect anomalies.
- **DDoS Mitigation**: Clients MAY cache configuration objects internally and rate-limit `sil_config` requests to prevent resource exhaustion. Implementations MAY impose a minimum response interval (e.g., 1 second).
