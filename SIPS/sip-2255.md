...
discussions-to: https://sila-magicians.org/t/web3-login-permissions/3583
status: Final
...
The `methodName` is the name of the method for which the permission is being requested (e.g. `sil_accounts`). The `caveatName` is the name of the caveat being applied to the permission (e.g. `requiredMethods`). The caveat value is the value of the caveat (e.g. `["signTypedData_v3"]`).
...
The `parentCapability` is the name of the method for which the permission is being requested (e.g. `sil_accounts`). The `date` is the timestamp of the request, in Unix time, and is optional.
...
The following example should prompt the user to approve the `sil_accounts` permission, and return the permission object if approved.
