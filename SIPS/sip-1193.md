...
discussions-to: https://github.com/sila-chain/SIPs/issues/2319
status: Final
...
If the Provider supports Sila RPC subscriptions, e.g. `sil_subscribe`, the Provider **MUST** emit the `message` event when it receives a subscription notification.
...
- [Initial discussion in `sila-chain/interfaces`](https://github.com/sila-chain/interfaces/issues/16)
- Deprecated Sila Magicians thread
...
`sil_` subscription methods and [`shh_` subscription methods](https://github.com/sila-chain/go-sila/wiki/Whisper-v6-RPC-API#shh_subscribe) rely on this event to emit subscription updates.
