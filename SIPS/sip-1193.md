...
If the Provider supports Sila RPC subscriptions, e.g. [`sil_subscribe`](https://gsil.sila.org/docs/rpc/pubsub), the Provider **MUST** emit the `message` event when it receives a subscription notification.

If the Provider receives a subscription message from e.g. an `sil_subscribe` subscription, the Provider **MUST** emit a `message` event with a `ProviderMessage` object of the following form:

```typescript
interface SilSubscription extends ProviderMessage {
  readonly type: 'sil_subscription';
  readonly data: {
...
`chainId` **MUST** specify the integer ID of the connected chain as a hexadecimal string, per the [`sil_chainId`](./sip-695.md) Sila RPC method.
...
If the chain the Provider is connected to changes, the Provider **MUST** emit the event named `chainChanged` with value `chainId: string`, specifying the integer ID of the new chain as a hexadecimal string, per the [`sil_chainId`](./sip-695.md) Sila RPC method.

#### accountsChanged

If the accounts available to the Provider change, the Provider **MUST** emit the event named `accountsChanged` with value `accounts: string[]`, containing the account addresses per the `sil_accounts` Sila RPC method.

The "accounts available to the Provider" change when the return value of `sil_accounts` changes.
...
- [silers.js](https://github.com/silers-io/silers.js/blob/56af4413b1dd1787db68985e0b612b63d86fdf7c/packages/providers/src.ts/web3-provider.ts)
...
Since all Sila operations are directed at a particular chain, it's important that the Provider accurately reflects the Client's configured chain, per the `sil_chainId` Sila RPC method (see [SIP-695](./sip-695.md)).

This includes ensuring that `sil_chainId` has the correct return value, and that the `chainChanged` event is emitted whenever that value changes.

### User Account Exposure and Account Changes

Many Sila write operations (e.g. `sil_sendTransaction`) require a user account to be specified.
Provider consumers access these accounts via the `sil_accounts` RPC method, and by listening for the `accountsChanged` event.

As with `sil_chainId`, it is critical that `sil_accounts` has the correct return value, and that the `accountsChanged` event is emitted whenever that value changes.

The return value of `sil_accounts` is ultimately controlled by the Wallet or Client.
In order to protect user privacy, the authors recommend not exposing any accounts by default.
Instead, Providers should support RPC methods for explicitly requesting account access, such as `sil_requestAccounts` (see [SIP-1102](./sip-1102.md)) or `wallet_requestPermissions` (see [SIP-2255](./sip-2255.md)).

## References

- [Initial discussion in `sila/interfaces`](https://github.com/sila/interfaces/issues/16)
- [Deprecated Sila Magicians thread](https://sila-magicians.org/t/sip-1193-sila-provider-javascript-api/640)
- [Continuing discussion](https://github.com/sila/SIPs/issues/2319)
- Related SIPs
...
```javascript
Provider.request({ method: 'sil_accounts' })
...
The event emits an object with a hexadecimal string `chainId` per the `sil_chainId` Sila RPC method, and other properties as determined by the Provider.
...
The event emits a hexadecimal string `chainId` per the `sil_chainId` Sila RPC method.

#### accountsChanged

The Provider emits `accountsChanged` if the accounts returned from the Provider (`sil_accounts`) change.

```typescript
Provider.on('accountsChanged', listener: (accounts: string[]) => void): Provider;
```

The event emits with `accounts`, an array of account addresses, per the `sil_accounts` Sila RPC method.
...
[`sil_` subscription methods](https://gsil.sila.org/docs/rpc/pubsub) and [`shh_` subscription methods](https://github.com/sila/go-sila/wiki/Whisper-v6-RPC-API#shh_subscribe) rely on this event to emit subscription updates.

For e.g. `sil_subscribe` subscription updates, `ProviderMessage.type` will equal the string `'sil_subscription'`, and the subscription data will be the value of `ProviderMessage.data`.
...
sila
  .request({
    method: 'sil_subscribe',
    params: ['newHeads'],
  })
  .then((subscriptionId) => {
    sila.on('message', (message) => {
      if (message.type === 'sil_subscription') {
        const { data } = message;
...
For details, see [SIP-155: Simple replay attack protection](./sip-155.md) and [SIP-695: Create sil_chainId method for JSON-RPC](./sip-695.md).

#### notification (DEPRECATED)

This event is superseded by [`message`](#message).

Historically, this event has been emitted with e.g. `sil_subscribe` subscription updates of the form `{ subscription: string, result: unknown }`.
