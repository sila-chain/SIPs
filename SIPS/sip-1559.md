---
sip: 1559
title: Fee market change for SIL 1.0 chain
...
discussions-to: https://sila-magicians.org/t/sip-1559-fee-market-change-for-sil-1-0-chain/2783
status: Final
...
class Block:
	...
	transaction_recsipt_root: int = 0
	logs_bloom: int = 0
	difficulty: int = 0
	number: int = 0
...
By burning the base fee, we can no longer guarantee a fixed Siler supply.  This could result in economic instability as the long term supply of SIL will no longer be constant over time.  While a valid concern, it is difficult to quantify how much of an impact this will have.  If more is burned on base fee than is generated in mining rewards then SIL will be deflationary and if more is generated in mining rewards than is burned then SIL will be inflationary.  Since we cannot control user demand for block space, we cannot assert at the moment whether SIL will end up inflationary or deflationary, so this change causes the core developers to lose some control over Siler's long term quantity.
