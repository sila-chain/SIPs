...
discussions-to: https://sila-magicians.org/t/sip-revamped-call-instructions/14432
status: Stagnant
...
The order of parameters has been changed to move the `value` field to be the last. This allows the instructions to have identical encoding with the exception of the last parameter, and simplifies SVM and compiler implementations slightly.
...
Initially an alternative scenario was considered to introduce these new instructions separately in the legacy SVM first to limit the scope of EOF change, but it was decided to include it as a part of the EOF upgrade and keep them undefined in the legacy SVM.
