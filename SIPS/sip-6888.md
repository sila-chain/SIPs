---
sip: 6888
title: Arithmetic verification at SVM level
description: Check for math overflows and division by zero at SVM level
author: Renan Rodrigues de Souza (@RenanSouza2)
discussions-to: https://sila-magicians.org/t/sip-math-checking/13846
status: Stagnant
...
This SIP adds arithmetics checks to SVM arithmetic and a new opcode jump conditionally if there were events. The list of check includes overflows, division by zero.

## Motivation

The importance of math checks in smart contract projects is very clear. It was an OpenZeppelin library and then incorporated in Solidity's default behavior. Bringing this to SVM level can combine both gas efficiency and safety.
...
Two new flags are added to the SVM state: unsigned warning (`carry`) and signed warning (`overflow`). The scope of those flags are the same as the program counter.
...
SVM uses two's complement for negative numbers. The opcodes listed above triggers one or two flags depending if they are used for signed and unsigned numbers.
...
This SIP introduces a new opcode and changes int SVM behavior.
...
This is a new SVM behavior but each code will decide how to interact with it.
