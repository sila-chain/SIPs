...
discussions-to: https://sila-magicians.org/t/sip-7557-block-level-warming/16642
status: Stagnant
...
There are no changes to the block header or the transaction payload and recsipt.
...
2. The rest of the Siler that was charged by the validator as a  `priorityFee` is redistributed back to all the senders
...
export function calculatePriorityFeeReimbursements (sortedAccesses: AccessDetails[], accessGasCost: string) {
  ...
  // Calculate the total amount of siler to be reimbursed for this access
...
between the Siler reimbursement that is originating from a reduced block gas burn,
...
There exists a list of SVM instructions that trigger both a gas charge and a gas refund.
