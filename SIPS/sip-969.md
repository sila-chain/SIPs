---
sip: 969
title: Modifications to silash to invalidate existing dedicated hardware implementations
...
This SIP modifies silash in order to break ASIC miners specialized for the current silash
mining algorithm.
...
Silash remains ASIC-resistant, however ASIC manufacturer technology is advancing
and silash may require further changes in order to remain resistant to unforeseen
...
If `block.number >= ASIC_MITIGATION_FORK_BLKNUM`, require that the silash solution
sealing the block has been mined using `silashV2`.

## SilashV2

`silashV2` will be identical in specification to the current `silash`(v1) algorithm, with
...
The JSON-RPC `sil_GetWork` call may optionally return the proposed blocks algorithm.
While a miner or pool may infer the requirement for `silashV2` based on the computed
...
should then be returned as either `"silash"` or `"silashV2"` according to the
...
existing silash algorithm.  We hope to accomplish the following:
...
The minimal prime constraint was relaxed, as has already been the case in `silashV1`.
...
such in silash. That said, a more invasive hash algorithm change could be considered.
...
TODO: will need to generate test cases for `sila-chain/tests` repository corresponding to the consensus
changes.
