...
The current methods used to send transactions from the user wallet and check their status are `sil_sendTransaction` and `sil_getTransactionReceipt`.
...
Today, developers want to send multiple calls batched tosilaer in a single RPC call, which many smart accounts can, in turn, execute atomically in a single transaction. Developers also want to make use of features afforded by new transaction formats, like paymasters in [SRC-4337](./sip-4337.md) transactions. `sil_sendTransaction` offers no way to do these things.
...
Apps may begin using these first three methods immediately, falling back to `sil_sendTransaction` and `sil_getTransactionReceipt` when they are not available.
...
Note that the `receipts` objects of this method's response is a strict subset of the object returned by `sil_getTransactionReceipt`.
