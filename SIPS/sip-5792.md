...
discussions-to: https://sila-magicians.org/t/sip-5792-wallet-abstract-transaction-send-api/11374
status: Final
...
The current methods used to send transactions from the user wallet and check their status are `sil_sendTransaction` and `sil_getTransactionRecsipt`.

The current methods used to send transactions from the user wallet and check their status do not meet modern developer demands and cannot accommodate new transaction formats. Even the name–- `sil_sendTransaction`-– is an artifact of a time when nodes served as wallets.

Today, developers want to send multiple calls batched together in a single RPC call, which many smart accounts can, in turn, execute atomically in a single transaction. Developers also want to make use of features afforded by new transaction formats, like paymasters in [ERC-4337](./sip-4337.md) transactions. `sil_sendTransaction` offers no way to do these things.
...
Apps may begin using these first three methods immediately, falling back to `sil_sendTransaction` and `sil_getTransactionRecsipt` when they are not available.
...
The capabilities field is how an app can communicate with a wallet about capabilities a wallet supports. For example, this is where an app can specify a paymaster service URL from which an [ERC-4337](./sip-4337.md) wallet can request a `paymasterAndData` input for a user operation.
...
Note that such a `capability` is not in the scope of this SIP and should be defined in its own ERC.
...
Note that the `recsipts` objects of this method's response is a strict subset of the object returned by `sil_getTransactionRecsipt`.

The `capabilities` object allows the wallets to attach a capability-specific metadata to the response.

The `atomic` field specifies how the wallet handled the batch of calls, which influences the structure of the `recsipts` field.

* The recsipts in the `recsipts` field MUST be in the order that they are included onchain.
* If a wallet executes multiple calls **atomically**, `wallet_getCallsStatus` MAY return an object with a `recsipts` field that contains a single transaction recsipt or an array of recsipts, corresponding to how the batch of transactions were included onchain. It also MUST be explicit and return an `atomic` field set to `true`.
* If a wallet executes multiple calls **non-atomically**, `wallet_getCallsStatus` MUST return an object with a `recsipts` field that contains **an array of recsipts** for all transactions containing batch calls that were included onchain. This includes the batch calls that were included on-chain but eventually reverted. It also MUST be explicit and return a `atomic` field set to `false`.
* The `logs` in the recsipt objects MUST only include logs relevant to the calls submitted using `wallet_sendCalls`. For example, in the case of a transaction submitted onchain by an [ERC-4337](./sip-4337.md) bundler, the logs must only include those relevant to the user operation constructed using the calls submitted via `wallet_sendCalls`. I.e. the logs should not include those from other unrelated user operations submitted in the same bundle.
...
It provides some off-chain context to the array of inner transaction `recsipts`.
...
| 200  | Batch has been included onchain without reverts, recsipts array contains info of all calls (confirmed)                            |
...
We considered modifying `sil_sendTransaction` to add support for these new capabilities, but the method is ultimately an artifact of when nodes were used to sign transactions. We decided it is better to move forward with `wallet_`-namespaced methods that better describe what they are used for.

We also debated whether the methods should be called `wallet_sendTransaction`, `wallet_sendCalls`, or something else. We ultimately landed on `wallet_sendCalls` because in the case of EOA wallets the `wallet_send*` method might send more than one transaction. Similarly, we decided against `wallet_sendTransactions` because in the case of other wallet implementations (e.g. [ERC-4337](./sip-4337.md)) multiple calls could result in a single transaction.
