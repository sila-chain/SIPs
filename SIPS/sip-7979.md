---
sip: 7979
title: Call and Return Opcodes for the SVM
...
discussions-to: https://sila-magicians.org/t/sip-7979-call-and-return-opcodes-for-the-evm/24615
status: Draft
...
This is a minimal change to the SVM to support calls and returns.

This proposal introduces three new control-flow instructions to the SVM:
...
The SVM currently lacks explicit call and return instructions.  Instead, calls and returns must be synthesized using the dynamic `JUMP` instruction, which takes its destination from the stack.  This creates two fundamental problems:
...
For detailed historical context, technical foundations of static control flow, and their impact on Sila's scaling roadmap, see [SIP-8173](./sip-8173.md): "Foundations of SVM Control Flow."
...
The SVM's machine state includes a *data stack* of 256-bit words, at most 1024 deep, and the program counter, *PC*, whose value is a position in the code — the index of the next byte to execute.  This SIP adds a *return stack* of return addresses, pushed only by `CALLSUB`, popped only by `RETURNSUB`, and not otherwise accessible to SVM code.
...
* *The description above lays out the semantics of these instructions in terms of a return stack.  But the actual state of the return stack is not observable by SVM code or consensus-critical to the protocol.  (For example, a node implementer may code `CALLSUB` to unobservably push the PC on the return stack rather than PC + 1, which is allowed so long as `RETURNSUB` observably returns control to the PC + 1 location.)*
...
Constraints on `MAGIC` code MUST be validated at `CREATE` time, in time and space linear in the size of the code.  To this end, a canonical SVM implementation of the validation algorithm MUST be placed on the blockchain and identified in the `MAGIC` header.  This validation code or its equivalent MUST be run on the *output* of the initialization code *before* it is executed by the interpreter, and *failure of validation is an exceptional halting state*.
...
The above is a purely *semantic* specification, placing no constraints on the *syntax* of bytecode beyond being an array of opcodes and immediate data.  Subroutines here *are not* contiguous sequences of bytecode: they are subgraphs of the bytecode's full control-flow graph.  The SVM is a simple state machine, and the control-flow graph for a program represents every possible change of state.  Each instruction simply advances the machine one state, and the instructions and state have minimal syntactic structure.  We only promise that *valid* code will not, as it were, jam up the gears of the machine.
...
With no syntactic constraints and minimal semantic constraints, we maximize opportunities for optimizations, including call elimination, multiple-entry calls, efficient register allocation, and inter-procedural optimizations.  Since we want to support online compilation of SVM code to native code, it is crucial that the SVM code be as well optimized as possible by high-level-language compilers — upfront and offline.
...
These changes are backwards compatible.  The new opcodes are available in both ordinary and `MAGIC` code.  Opcode semantics *are not* affected by whether the contract is `MAGIC`, so *valid* code will execute identically in any contract.  Validation of `MAGIC` code is done before the interpreter runs, such that the interpreter never sees `MAGIC` code that is not *valid*.  There are *no changes* to the semantics of existing SVM code.  (With the caveat that code with unspecified behavior might behave in different, unspecified ways.  Such code was always broken.)
...
These changes do not preclude running the SVM in zero knowledge; neither do they foreclose EOF, RISC-V, or other changes.
...
The following is a Vyper implementation of the validation algorithm for `MAGIC` code.  It validates SVM bytecode against the five constraints defined above in time and space linear in the size of the code.
...
# @notice Validates that SVM bytecode satisfies the constraints for MAGIC code
...
# Maximum SVM stack depth, as a signed bound on the summary values.
...
def opcode_info(op: uint8) -> (uint256, int256, int256, bool):
    ...
    Table current as of the Prague fork, plus this SIP's CALLSUB,
    ...
    # the Prague fork plus this SIP's three, is elided here; it is in
