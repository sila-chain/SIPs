...
author: Jame (@ZWJKFLC)
discussions-to: https://sila-magicians.org/t/sip-7637-extcodehash-optimize/18946
status: Stagnant
...
```python
def extcodehash(svm: Svm) -> None:
    address = to_address(pop(svm.stack))
    charge_gas(svm, GAS_CODE_HASH)
    account = get_account(svm.env.state, address)
