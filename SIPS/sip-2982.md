...
description: Phase 0 of the release schedule of Serenity, a series of updates to Sila a scalable, proof-of-stake consensus
author: Danny Ryan (@djrtwo), Vitalik Buterin (@vbuterin)
discussions-to: https://sila-magicians.org/t/serenity-phase-0-sip/4621
status: Final
type: Informational
created: 2020-09-15
---
...
This SIP specifies Phase 0 of Serenity (sil2), a multi-phased upgrade to the consensus mechanism for SilaMainnet. In Phase 0, the existing PoW chain and mechanics are entirely unaffected, while a PoS chain -- the beacon chain -- is built in parallel to serve as the core of the upgraded consensus. In subsequent phases, the beacon chain is enhanced to support and secure the consensus of a number of parallel shard chains, ultimately incorporating current SilaMainnet as one of those shards.

At the core of the beacon chain is a proof of stake consensus mechanism called Casper the Friendly Finality Gadget (FFG) and a fork-choice rule called Latest Message Driven Greedy Heaviest Observed Sub-Tree (LMD-GHOST). Phase 0 is centered primarily around the mechanics and incentives of validators executing these algorithms. The detailed specifications for sil2 are contained in an independent repository from this SIP, and safety and liveness proofs can be found in the [Combining GHOST and Casper](../assets/sip-2982/arxiv-2003.03052-Combining-GHOST-and-Casper.pdf) paper. To avoid duplication, this SIP just references relevant spec files and releases.

Early phases of sil2 are executed without any breaking consensus changes on current SilaMainnet. This SIP serves to document the bootstrapping of this consensus mechanism and note the path for sil2 to supplant Sila's current proof-of-work (PoW) consensus.

## Motivation

Sil2 aims to fulfill the original vision of Sila to support an efficient, global-scale, general-purpose transactional platform while retaining high cryptoeconomic security and decentralization.
...
To provide more scale to Sila, while not inducing a restrictively high burden on both consumer and consensus nodes, sil2 introduces a "sharded" solution in which a number of blockchain shards -- each of similar capacity to SilaMainnet today -- run in parallel under a unified consensus mechanism. The core consensus (the beacon chain) and a small number of these shards can be processed via a single consumer machine, while the aggregate of the system provides much higher capacity.
...
Phase 0 is designed to require _no breaking consensus changes_ to existing SilaMainnet. Instead, this is the bootstrapping a new PoS consensus that can, once stable, supplant the current PoW consensus.
...
This SIP provides a high level view on the Phase 0 mechanisms, especially those that are relevant to SilaMainnet (e.g. the deposit contract) and users (e.g. validator mechanics and sil2 issuance). The extended and low level details remain in the `consensus-specs` repository
...
_Note:_ Sil2 has many more Phase 0 configuration parameters but the majority are left out of this SIP for brevity.

### Validator deposit contract

In Phase 0, sil2 uses a contract deployed on SilaMainnet -- the Deposit Contract -- at `DEPOSIT_CONTRACT_ADDRESS` to onboard validators into the PoS consensus of the beacon chain.

To participate in the PoS consensus, users submit validator deposits to the deposit contract. The beacon chain comes to consensus on the state of this contract and processes new validator deposits. This uni-directional deposit mechanism is the only technical link between the two components of the system (SilaMainnet and beacon chain) in Phase 0.

### Beacon chain and validator mechanics

Users who choose to participate in sil2 consensus deposit SIL collateral into the deposit contract in order to be inducted into the beacon chain validator set. From there, these validators are responsible for constructing the **beacon chain** (note that these consensus participants in PoS are akin to miners in PoW).
...
To incentivize validators to deposit siler collateral and participate in the sil2 consensus, we propose that rewards (in the form of Sila's native asset, siler) be regularly issued to consensus participants. Due to the beacon chain operating in parallel to the existing PoW chain in early phases of sil2, this issuance is _in addition to_ any PoW rewards until the existing chain is merged into sil2 as a shard.

The amount of siler issued to validators on the beacon chain is proportional to the square root of the total siler deposited. This issuance curve was chosen as a more stable and sustainable curve to the two obvious alternatives -- fixed total issuance and fixed issuance per siler staked. For a more technical discussion on this choice see [here](../assets/sip-2982/ef-Discouragement-Attacks.pdf).

In sil2, this curve is parameterized by `BASE_REWARD_FACTOR` in the context of slot time and epoch length. Below is the issuance curve as a function of siler staked, along with a table of examples for illustration. Note, all figures shown are annualized.
...
_`INACTIVITY_PENALTY_QUOTIENT` is configured initially to four times its final value_. This results in a slower inactivity leak during times of non-finality, which means the chain is less responsive to such an event. If there is an extended time of non-finality during the early months of sil2, it is far more likely to be due to technical issues with client software rather than some sort of global catastrophic event.

_`PROPORTIONAL_SLASHING_MULTIPLIER` is configured initially to one-third of its final value_. This results in a lower accountable safety margin in the event of an attack. If any validators are slashed in the early months of sil2, it is far more likely to be the result of user mismanagement of keys and/or issues with client software than an organized attack.

_`MIN_SLASHING_PENALTY_QUOTIENT` configured initially to four times its final value_. This results in a lower guaranteed minimum penalty for a slashable offense and thus reduces the baseline punitive incentive to keep an individual validator's system secure. As with `PROPORTIONAL_SLASHING_MULTIPLIER`, slashings during the early months of sil2 are far more likely to be due to user mismanagement, or issues with client software, than an organized attack.
...
The reason to not take a pure layer 2 approach is that pure layer 2 scaling can only be done either with trust-based solutions (not desirable), or with channels or plasma (which have inherent limitations and cannot support the full SVM.
...
See https://github.com/sila/sil2.0-specs/blob/dev/ssz/merkle-proofs.md for a description of generalized indices, and how they allow for very easy verification of Merkle proofs "poking into" arbitrary positions in an object by representing paths as an integer or bitfield.

### The validator lifecycle

#### Depositing

A validator deposits by sending a transaction that calls a function on the deposit contract on the sil1 chain (eventually, a way to deposit from inside sil2 will also be added). A deposit specifies:
...
The deposit contract maintains a Merkle root of all deposits made so far. Once a merkle root containing the deposit gets included into the sil2 chains (via the Sil1Data voting mechanism), an sil2 block proposer can submit a Merkle proof of the deposit and start the deposit process.
...
The reason for the entrance/exit queue limits is to ensure that the validator set cannot change too quickly between any two points in time, which ensures that finality guarantees still remain between two chains as long as a validator logs on often enough (once every ~1-2 months if $|V| \ge 262144$). See here Sila Research for rationale (and specifically why to use a rate limit instead of a fixed waiting time) and Sila Research for how to calculate safety margin for a client that has been offline for some amount of time.
...
Although this SIP does not introduce any immediate changes to the current SilaMainnet, this SIP lays the groundwork for future backwards incompatibilities through the introduction of the new sil2 consensus mechanism in which Sila will be integrated in subsequent phases. To secure this mechanism, users move siler into the beacon chain and additional siler is issued. This SIP is a commitment to this path being canonical, as well as directly informing the future and roadmap of SilaMainnet.

## Security Considerations

Sil2 is a major overhaul of the Sila's core consensus from PoW to a sharded PoS. There are inherent risks in this migration but there is extensive research literature analyzing security and trade-offs. _The following only represents a high level selection of the resources available:_
