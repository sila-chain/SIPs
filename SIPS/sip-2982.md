...
This SIP specifies Phase 0 of Serenity (sil2), a multi-phased upgrade to the consensus mechanism for Sila sila-mainnet. In Phase 0, the existing PoW chain and mechanics are entirely unaffected, while a PoS chain -- the beacon chain -- is built in parallel to serve as the core of the upgraded consensus. In subsequent phases, the beacon chain is enhanced to support and secure the consensus of a number of parallel shard chains, ultimately incorporating current Sila sila-mainnet as one of those shards.
...
Early phases of sil2 are executed without any breaking consensus changes on current Sila sila-mainnet. This SIP serves to document the bootstrapping of this consensus mechanism and note the path for sil2 to supplant Sila's current proof-of-work (PoW) consensus.
...
This SIP provides a high level view on the Phase 0 mechanisms, especially those that are relevant to Sila sila-mainnet (e.g. the deposit contract) and users (e.g. validator mechanics and sil2 issuance). The extended and low level details remain in the `consensus-specs` repository
...
In Phase 0, sil2 uses a contract deployed on Sila sila-mainnet -- the Deposit Contract -- at `DEPOSIT_CONTRACT_ADDRESS` to onboard validators into the PoS consensus of the beacon chain.
...
To incentivize validators to deposit sila collateral and participate in the sil2 consensus, we propose that rewards (in the form of Sila's native asset, sila) be regularly issued to consensus participants. Due to the beacon chain operating in parallel to the existing PoW chain in early phases of sil2, this issuance is _in addition to_ any PoW rewards until the existing chain is merged into sil2 as a shard.
...
In sil2, this curve is parameterized by `BASE_REWARD_FACTOR` in the context of slot time and epoch length. Below is the issuance curve as a function of sila staked, along with a table of examples for illustration. Note, all figures shown are annualized.
...
These requirements tosilaer make a Merkle tree structure the obvious choice.

#### Generalized indices

See https://github.com/sila-chain/consensus-specs/blob/dev/ssz/merkle-proofs.md for a description of generalized indices, and how they allow for very easy verification of Merkle proofs "poking into" arbitrary positions in an object by representing paths as an integer or bitfield.
...
The reason for the entrance/exit queue limits is to ensure that the validator set cannot change too quickly between any two points in time, which ensures that finality guarantees still remain between two chains as long as a validator logs on often enough (once every ~1-2 months if $|V| \ge 262144$). See here  for rationale (and specifically why to use a rate limit instead of a fixed waiting time) and  for how to calculate safety margin for a client that has been offline for some amount of time.
