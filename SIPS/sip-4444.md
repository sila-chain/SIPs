...
We suggest that a specialized "full sync" client is built. The client is a shim that pieces tosilaer different releases of execution engines and can import historical blocks to validate the entire Sila chain from genesis and generate all other historical data.
...
In the first phase, clients don't prune historical data by default. They introduce a command line option similar to sila's `--txlookuplimit` that users can use if they want to prune historical data.
