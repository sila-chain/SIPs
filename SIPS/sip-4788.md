---
sip: 4788
title: Beacon block root in the SVM
description: Expose beacon chain roots in the SVM
...
discussions-to: https://sila-magicians.org/t/sip-4788-beacon-root-in-evm/8281
status: Final
...
Exposing these roots inside the SVM allows for trust-minimized access to the consensus layer.
...
```python
if svm.caller == SYSTEM_ADDRESS:
    set()
else:
    get()

def get():
    if len(svm.calldata) != 32:
        svm.revert()

    if to_uint256_be(svm.calldata) == 0:
        svm.revert()

    timestamp_idx = to_uint256_be(svm.calldata) % HISTORY_BUFFER_LENGTH
    timestamp = storage.get(timestamp_idx)

    if timestamp != svm.calldata:
        svm.revert()
...
def set():
    timestamp_idx = to_uint256_be(svm.timestamp) % HISTORY_BUFFER_LENGTH
    root_idx = timestamp_idx + HISTORY_BUFFER_LENGTH

    storage.set(timestamp_idx, svm.timestamp)
    storage.set(root_idx, svm.calldata)
```
...
Clients may decide to omit an explicit SVM call and directly set the storage values. Note: While this is a valid optimization for Sila sila-mainnet, it could be problematic on non-sila-mainnet situations in case a different contract is used.
