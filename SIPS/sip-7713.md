---
sip: 7713
title: Box type for SIP-712 messages
description: A mechanism for SIP-712 messages to contain parameters of arbitrary type
author: Francisco Giordano (@frangio)
discussions-to: https://sila-magicians.org/t/sip-7713-box-types-for-sip-712-messages/20092
status: Stagnant
...
A recent example is found in [ERC-7683](./sip-7683.md), which defines a struct with the following member:
...
Another example is found in recent efforts to make [ERC-1271](./sip-1271.md) signatures secure against replay. Achieving this without making the message contents opaque to the signer requires embedding an application's SIP-712 message inside an outer message that binds it to a specific account. The type of the outer message depends on the type of the inner message, and making the type reproducible by the smart contract account on-chain for verification requires an inefficient scheme to communicate the string-encoded type of the inner message as a part of the signature.
