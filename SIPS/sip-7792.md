...
description: Scheme to make the sil_getLogs response verifiable
author: Etan Kissling (@etan-status), Gajinder Singh (@g11tech), Vitalik Buterin (@vbuterin)
discussions-to: https://sila-magicians.org/t/sip-7792-verifiable-logs/21424
status: Stagnant
...
```python
def accumulate_log(svm: Svm, entry_root: Bytes32, key: Bytes32):
    root = hashlib.sha256()
    root.update(entry_root)
    root.update(sload(svm.env.state, LOG_CONTRACT_ADDRESS, key))
    sstore(svm.env.state, LOG_CONTRACT_ADDRESS, key, root.digest())

def track_log(svm: Svm, entry: LogEntry) -> None:
    ...
    accumulate_log(svm, entry_root, key)
...
The `sil_getLogs` response format is extended to include:

- `blockTimestamp`: `QUANTITY` - The timestamp field of the block referred to by `blockHash`

### Verification

For `sil_getLogs(address, topics, fromBlock, toBlock)`, the response data can be verified for correctness and completion by obtaining:
...
3. Historical log accumulator at `parentBlock` based on given filters (validated with `sil_getProof`)
4. Log accumulator at `toBlock` based on given filters (validated with `sil_getProof`)
...
Making the `sil_getLogs` response verifiable adds the necessary security attributes to enable wallets to transition away from relying on trusted data providers, ultimately improving the wallet's privacy guarantees as it is no longer subject to the privacy policy of any given provider.
...
It is still possible to process `sil_getLogs` responses from trusted servers as is, without verifying them. Client applications with strict response validation may need to be updated to allow the additional `blockTimestamp` field.

## Security Considerations

This scheme reuses existing `sil_getProof` and SSZ Merkle proofs; it does not introduce new security risks.
