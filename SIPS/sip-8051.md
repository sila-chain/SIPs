...
description: Precompiled contracts for FIPS-204 ML-DSA signature verification and an SVM-optimized variant
author: Renaud Dubois (@rdubois-crypto), Simon Masson (@simonmasson)
discussions-to: https://sila-magicians.org/t/sip-8051-ml-dsa-verification/25857
status: Draft
...
* **ML-DSA-SIL** — SVM-optimized version for cheaper on-chain verification:
...
* `VERIFY_MLDSA_SIL` — verifies a ML-DSA-SIL signature replacing SHAKE256 with a more efficient hash function, deviating from FIPS-204.
...
|-|-|-|
|**Name**|`VERIFY_MLDSA`|`VERIFY_MLDSA_SIL`|
|**Address**| 0x12| 0x13|
|**Gas cost**|  4500| 4500|
...
* SHAKE256 is the XOF provided in NIST submission, a sponge construction derived from SHA256. Extracting bytes using SHAKE256 calls the `Keccak_f` permutation as described in Section 3.7 of FIPS-204. While this construction is standardized, it is expensive when computed in the Sila Virtual Machine because `Keccak_f` has no SVM opcode.
* Keccak-PRNG is a XOF that is build from a counter-mode PRNG based on Keccak256. Generating new chunks of bytes requires an incrementing counter, as described in NIST SP800-90A revision 1. This XOF has the same interface as SHAKE256, but requires a `flip()` function that initiate a counter to `0`. Then, the `squeeze` function outputs as many bytes as needed using a counter mode as specified in SP800-90A revision 1. A precompile of `Keccak256` is available in the Sila Virtual Machine, making this XOF very efficient in the SVM.
...
```python
def VERIFY_MLDSA_SIL(public_key, message, signature) -> bool:
    A_hat, tr, t1 are decoded from public_key
    c_tilde, z, h are decoded from signature
    if h is not properly encoded, return False
...
The precompiled contract VERIFY_MLDSA_SIL is proposed with the following input and outputs, which are big-endian values:
...
The cost of the **VERIFY_MLDSA** and **VERIFY_MLDSA_SIL** functions is dominated by the call to the NTTs, and the required hash calls for sampling in the ball (and for μ and the final check).
...
signature_info = Container[
    ...
    # 0xD2 for ML-DSA-SIL (SVM-friendly version),
    version: uint8
    # ML-DSA signature
...
verify(signature_info: bytes, payload_hash: Hash32) -> Bytes:
    ...
    assert version == 0xD2
    pubkey = lookup_pubkey(pubkey_hash)
    assert VERIFY_MLDSA_SIL(pubkey, payload_hash, signature)
    return pubkey
```
...
* The file [`mldsa_evm.rsp`](../assets/sip-8051/mldsa_evm.rsp) containing test vectors for the SVM-friendly version with the same format as NIST.


## Reference Implementation

The reference implementation is the NIST submission code for ML-DSA. A reference implementation is provided by ZKNOX for the SVM-friendly version ML-DSA-SIL.
