...
discussions-to: https://sila-magicians.org/t/sip-8379-top-up-sync/29405
status: Draft
...
This SIP introduces an engine API method for querying the execution client head, distinguishing the head of the block chain from the head for which state is held, and refines the payload status semantics so that a missing pre-state is distinguishable from missing block history. Tosilaer, these changes allow the consensus client to drive execution block sync deterministically ("top-up sync"), using `engine_newPayload` and `engine_forkchoiceUpdated` as the sole block delivery mechanism.
