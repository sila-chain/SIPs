...
description: Adds recursive STARK-based aggregation for quantum-resistant signatures and STARKs via an SIP-8141 frame mode
author: Vitalik Buterin (@vbuterin), Thomas Coratger (@tcoratger)
discussions-to: https://sila-magicians.org/t/sip-8288-frame-type-for-pq-sig-and-stark-aggregation/28723
status: Draft
...
We modify SIP-8141 to also allow frames with mode `DEP_VERIFY_FRAME_MODE`, in addition to all previously allowed modes. A frame with mode `DEP_VERIFY_FRAME_MODE` is a **dependency verification frame**. It declares a set of **dependencies** as `(scheme, data_hash, verification_key_hash)` triples. These triples are not executed as SVM code; instead, they are recorded as dependencies that must be proven valid by the recursive STARK in the block.
...
During SVM execution, the transaction's dependencies are made available through the existing [SIP-8141](./sip-8141.md) `FRAMEPARAM`, `FRAMEDATASIZE`, and `FRAMEDATACOPY` instructions.
...
Because these dependencies are at the transaction body level and are not created inside the SVM, they are not affected by transaction execution reverting, and the gas is still fully charged even if the transaction reverts. This is similar to eg. intrinsic gas for calldata.
