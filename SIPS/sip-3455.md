...
discussions-to: https://sila-magicians.org/t/sip-3455-sudo-opcode/5860
status: Stagnant
...
In particular, at address zero there is approximately 17 billion USD in tokens and siler, according to silerscan.
...
It is a common mistake to send [ERC-20](./sip-20.md) tokens to the token address instead of the intended recipient.
...
Large amounts of users have accidentally locked up tokens and siler by losing their private keys.
...
Finally, there are many tokens and siler sitting in smart contracts locked due to a bug.
We could finally close SIP issue #156.
We cannot currently reclaim siler from stuck accounts.
`SUDO` fixes this.
...
If the call ended with `REVERT`, `INVALID`, or an SVM assertion, `0` is pushed.
