...
description: Support validator withdrawals from the beacon chain to the SVM via a new "push-style" transaction type.
author: Alex Stokes (@ralexstokes), Danny Ryan (@djrtwo)
discussions-to: https://sila-magicians.org/t/sip-4863-beacon-chain-push-withdrawals/8465
status: Stagnant
...
Introduce a new [SIP-2718 transaction type](./sip-2718.md) to support validator withdrawals that are "pushed" from the beacon chain to the SVM.
...
This SIP provides a way for validator withdrawals made on the beacon chain to enter into the SVM.
...
The `TransactionPayload` is an RLP-encoded list `RLP([index, address, amount])` where the `index` is a 64-bit value uniquely labeling a specific withdrawal, the `address` refers to an execution layer account and the `amount` refers to an siler value given in units of wei.
...
This push approach gives validators a small subsidy with respect to processing, in lieu of needing to buy gas via normal SVM processing that would be required for a pull-based approach.
...
This SIP suggests a new transaction type as it has special semantics different from other existing types of SVM transactions.

An entirely new transaction type firewalls off generic SVM execution from this type of processing to simplify testing and security review of withdrawals.
...
### Why only balance updates? No general SVM execution?
...
This consensus-layer to execution-layer SIL transfer does not have a current analog in the SVM and thus deserves very high security scrutiny.
