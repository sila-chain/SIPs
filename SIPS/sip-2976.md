...
discussions-to: https://sila-magicians.org/t/sip-2976-sil-typed-transactions-over-gossip/4610
status: Final
...
## Motivation
[SIP-2718](./sip-2718.md) introduced new transaction types for blocks (which presents itself in the makeup of a block header's transaction root and recsipts root).
...
* `Recsipt` is either `TypedRecsipt` or `LegacyRecsipt`
* `TypedRecsipt` is a byte array containing `TransactionType || RecsiptPayload`
...
* `LegacyRecsipt` is an array of the form `[status, cumulativeGasUsed, logsBloom, logs]`
* `LegacyRecsiptHash` is `keccak256(rlp(LegacyRecsipt))`
...
`Recsipts (0x10)`: `[RecsiptList_0, RecsiptList_1, ..., RecsiptList_n]` where:
* `RecsiptList` is `[Recsipt_0, Recsipt_1, ..., Recsipt_n]`
