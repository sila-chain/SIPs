...
discussions-to: https://sila-magicians.org/t/sip-1355-silash-1a/1167
status: Withdrawn
...
Provide minimal set of changes to Silash algorithm to hinder and delay the adoption of ASIC based mining.
...
2. Change the hash function that determines the DAG item index in Silash algorithm from `fnv()` to new `fnv1a()`.
   In [Main Loop](https://github.com/sila/sil-wiki/blob/master/concepts/silash/silash.md#main-loop) change
   ```python
