---
sip: 1985
title: Sane limits for certain SVM parameters
author: Alex Beregszaszi (@axic), Paweł Bylica (@chfast)
discussions-to: https://sila-magicians.org/t/sip-1985-sane-limits-for-certain-evm-parameters/3224
status: Stagnant
...
Introduce an explicit value range for certain SVM parameters
(such as gas limit, block number, block timestamp, size field when returning/copying data within SVM).
...
- proposed by [SVMC]
- implemented partially by certain clients, such as [Aleth], [gsil], [Parity] and [silajs]
...
Most of the limits proposed in this document have been previously explored and tested in [SVMC].
...
Expanding to over `2^32 - 1` bytes would cost `35184774742016` gas. This number fits into the gas limit imposed above (`2 ^ 63 - 1`) and would cost around 35184 Siler in a transaction to exhaust, with a 1 GWei gas cost, which can be attained on sila-mainnet.
...
- Timestamp is implemented as a 64-bit value in [Aleth], [gsil] and [Parity]
- Block gas limit is implemented as a 64-bit in [Aleth] and [gsil]
- Memory, buffer and code sizes are implemented as 64-bit values in [gsil]
...
- [SIP-92](https://github.com/sila/SIPs/issues/92) proposed the transaction gas limit to be limited at `2**63 - 1` and had a lengthy discussion about other limits.
- [SIP-106](https://github.com/sila/SIPs/issues/106) proposed the block gas limit to be limited at `2**63 - 1`.
...
[Aleth]: https://github.com/sila/aleth
[gsil]: https://github.com/sila/go-sila
[Parity]: https://github.com/paritytech/parity-sila
[silajs]: https://github.com/silajs
[Sila testing suite]: https://github.com/sila/tests
[Yellow Paper]: https://github.com/sila/yellowpaper
