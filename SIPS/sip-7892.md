...
description: Defines a mechanism for scaling Sila’s blob capacity via specialized hard forks that modify only blob-related parameters.
author: Mark Mackey (@silDreamer), Raúl Kripalani (@raulk)
discussions-to: https://sila-magicians.org/t/sip-7892-blob-parameter-only-hardforks/23018
status: Final
type: Informational
created: 2025-02-28
requires: 7840
...
When discovering and interfacing with peers, nodes MUST evaluate `nfd` alongside their existing consideration of the `ENRForkID::next_*` fields under the `sil2` key, to form a more accurate view of the peer's intended next fork.
