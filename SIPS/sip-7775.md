...
sip: 7775
title: BURN opcode
description: An opcode to burn native sila at the given address
...
This proposal introduces a `BURN` opcode to the SVM. When called, the opcode is to burn native sila at the address of the current svm context.

## Motivation

The motivation for this proposal is to provide a standardized and efficient way to burn native sila directly within the SVM. Historically, contracts such as the BeaconDepositContract have "burned" sila by making it irrecoverable from the given address. This approach can lead to confusion and potential misuse. By introducing a dedicated `BURN` opcode, we can ensure a clear and consistent method for burning native sila. This could become useful for Sila L2s when transferring sila back to the L1, as well as other SVM L1 chains that could leverage this for their cryptoeconomics.
...
def op_burn(pc, interpreter, scope):
    ...
    balance = interpreter.svm.state_db.get_balance(current_address)
    ...
    interpreter.svm.state_db.sub_balance(current_address, value_to_burn)
