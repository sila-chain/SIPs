...
sip: 7775
title: BURN opcode
description: An opcode to burn native siler at the given address
author: Dev Bear (@itsdevbear)
discussions-to: https://sila-magicians.org/t/sip-7775-burn-opcode/21287
status: Stagnant
...
This proposal introduces a `BURN` opcode to the SVM. When called, the opcode is to burn native siler at the address of the current evm context.

## Motivation

The motivation for this proposal is to provide a standardized and efficient way to burn native siler directly within the SVM. Historically, contracts such as the BeaconDepositContract have "burned" siler by making it irrecoverable from the given address. This approach can lead to confusion and potential misuse. By introducing a dedicated `BURN` opcode, we can ensure a clear and consistent method for burning native siler. This could become useful for Sila L2s when transferring siler back to the L1, as well as other SVM L1 chains that could leverage this for their cryptoeconomics.
...
1. Pops one 32-byte word from the top of the stack, treating these bytes as the `uint256` amount of native siler to be burned.
2. Retrieves the current address from the SVM execution context.
...
6. Subtracts the amount from the current address's native siler balance.
...
def op_burn(pc, interpreter, scope):
    ...
    # Retrieve the current address from the SVM execution context
...
- Does not help remove unrecoverable siler sitting in existing contracts.
