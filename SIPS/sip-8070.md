...
This SIP extends `engine_forkchoiceUpdatedV4` as defined in the Amsterdam Engine API specification by adding an optional `custodyColumns` parameter, allowing the CL to inform the EL of its current blob column custody set.
...
Evaluating for sensible values of $p$ and $k$ yields, where $D=50$ (Gsil's default mesh degree):
