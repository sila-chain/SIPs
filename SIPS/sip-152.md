...
discussions-to: https://github.com/sila-chain/SIPs/issues/152
status: Final
...
This SIP will enable the BLAKE2b hash function and other higher-round 64-bit BLAKE2 variants to run cheaply on the SVM, allowing easier interoperability between Sila and Zcash as well as other Equihash-based PoW coins.

## Abstract

This SIP introduces a new precompiled contract which implements the compression function `F` used in the BLAKE2 cryptographic hashing algorithm, for the purpose of allowing interoperability between the SVM and Zcash, as well as introducing more flexible cryptographic hash primitives to the SVM.
...
In contrast, the big-endian 32 byte semantics of the SVM are not conducive to efficient implementation of BLAKE2, and thus the gas cost associated with computing the hash on the SVM is disproportionate to the true cost of computing the function natively.

An obvious implementation would be a direct BLAKE2b hash function precompile. At first glance, a BLAKE2b precompile satisfies most hashing and interoperability requirements on the SVM. Once we started digging in, however, it became clear that any BLAKE2b implementation would need specific features and internal modifications based on different projects' requirements and libraries.
...
An initial implementation of the `F` function in Go, adapted from the standard library, can be found in our [Golang BLAKE2 library fork](https://github.com/keep-network/blake2-f). There's also an implementation of the precompile in our fork of go-sila.
