...
discussions-to: https://sila-magicians.org/t/sip-3860-limit-and-meter-initcode/7018
status: Final
...
Lastly, the size limit results in the nice-to-have property that SVM code size, code offset (`PC`), and jump offset fits a 16-bit value.
...
3. To simplify SVM engines by the explicit limits (code size, code offsets (`PC`), and jump offsets fit 16-bits).
...
The value of `INITCODE_WORD_COST` is selected based on performance benchmarks of differing worst-cases per implementation. The baseline for the benchmarks is the performance of `KECCAK256` hashing in sila 1.10.9, which matches the 70 Mgas/s gas limit target on a 4.0 GHz x86_64 CPU.

| SVM             | version | MB/s | B/CPUcycle | CPUcycle/B | cost of 1 B | cost of 32 B |
| --------------- | ------- | ---- | ---- | ---- | ---- | ---- |
| sila/KECCAK256  | 1.10.9  |  357 |  1.8 |  0.6 |  0.2 |  6.0 |
| sila            | 1.10.9  | 1091 |  5.5 |  0.2 |  0.1 |  2.0 |
| svmone/Baseline | 0.8.2   |  727 |  3.7 |  0.3 |  0.1 |  2.9 |
| svmone/Advanced | 0.8.2   |  155 |  0.8 |  1.3 |  0.4 | 13.8 |

### Gas cost per word (32-byte chunk)

We have chosen the cost of 2 gas per word based on Sila's implementation and comparing with `KECCAK256` performance. This means the per byte cost is `0.0625`. While fractional gas costs are not permitted in the SVM, we can approximate it by charging per-word.
