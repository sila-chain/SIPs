...
description: Gas accounting by SVM resource, increasing throughput and improving resource usage controls, with minimal changes to the protocol and UX
author: Maria Silva (@misilva73), Davide Crapis (@dcrapis), Anders Elowsson (@anderselowsson), Toni Wahrstätter (@nerolation)
discussions-to: https://sila-magicians.org/t/sip-8011-multidimensional-gas-metering/25260
status: Draft
...
Under the multidimensional metering model, SVM operations are assigned a cost vector whose components correspond to their gas cost on each resource dimension. Each cost has 6 dimensions: `gas_cost_vector = (compute_cost, access_cost, size_cost, memory_cost, state_cost, history_cost)`.
...
[`calculate_gas_extend_memory`]: https://github.com/sila/execution-specs/blob/91824145ef62da61803edf9a764fd8f3662794c0/src/sila/osaka/vm/gas.py#L167
...
[`calculate_intrinsic_cost`]: https://github.com/sila/execution-specs/blob/91824145ef62da61803edf9a764fd8f3662794c0/src/sila/osaka/transactions.py#L571
<-- TODO -->

### Gas accounting

Besides replacing the single gas cost with a 6-dimensional cost vector, the gas accounting during block execution also changes. Instead of keeping track of a single total of gas used, the SVM stores a 6-dimensional vector with how much gas units was spent on each resource until that point in the execution.

After executing the ith SVM operation of a transaction, say `OP_i`, with a cost vector `gas_cost_vector_i = (compute_cost_i, access_cost_i, size_cost_i, memory_cost_i, state_cost_i, history_cost_i)`, the following variables are updated:
...
4. **Long-term Storage**: Tracks persistent changes to disk usage. There are two dimensions here:
    ...
    - History Growth: Delta in disk space used for historical recsipts, logs, etc.
