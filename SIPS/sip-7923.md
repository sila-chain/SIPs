...
discussions-to: https://sila-magicians.org/t/sip-linearize-memory-costing/23290
status: Draft
...
This SIP replaces the quadratic memory model in the SVM with a linear, page-based costing model. Memory is virtually addressable. Page allocation is included in the cost model. After applying this SIP, memory limits are invariant to the state of the message call stack.

## Motivation

The SVM currently uses a quadratic pricing model for its memory. This was originally put in place to defend against DoS attacks. However, the memory model has several drawbacks.
...
There is a desire among client implementations to be able to enforce global limits separately from the gas limit due to DoS reasons. For example, RPC providers may be designed to allow many concurrent `sil_call` computations with a much higher gas limit than on sila-mainnet. Not implicitly tying the memory limit to the gas limit results in one less vector for misconfiguration. That is not to say that in the future, a clean formula cannot be created which allows the memory limit to scale with future hardware improvements (e.g., proportional to the sqrt of the gas limit), but to limit the scope of things that need to be reasoned about for this SIP, the hard limit is introduced.
...
A ~60-line reference implementation is provided below. It is implemented as a patch against the `py-svm` codebase at commit sila-chain/py-svm@fec63b8c4b9dad9fcb1022c48c863bdd584820c6. (This is a reference implementation, it does not, for example, contain fork choice rules).

```diff
diff --git a/sila/vm/computation.py b/sila/vm/computation.py
index bf34fbee..db85aee7 100644
--- a/sila/vm/computation.py
+++ b/sila/vm/computation.py
...
diff --git a/sila/vm/forks/frontier/computation.py b/sila/vm/forks/frontier/computation.py
index 51666ae0..443f82b5 100644
--- a/sila/vm/forks/frontier/computation.py
+++ b/sila/vm/forks/frontier/computation.py
...
diff --git a/sila/vm/logic/memory.py b/sila/vm/logic/memory.py
index 806dbd8b..247b3c74 100644
--- a/sila/vm/logic/memory.py
+++ b/sila/vm/logic/memory.py
...
diff --git a/sila/vm/memory.py b/sila/vm/memory.py
...
diff --git a/sila/vm/transaction_context.py b/sila/vm/transaction_context.py
index 79b570e9..5943f897 100644
--- a/sila/vm/transaction_context.py
+++ b/sila/vm/transaction_context.py
