...
discussions-to: https://sila-magicians.org/t/sip-8371-rowdas-distributed-blobspace-reconstruction/29320
status: Draft
...
The exact wire format — SSZ containers, bitmap encoding, and GroupID derivation — is to be defined in the sila-chain/consensus-specs repository, following the approach of [SIP-8136](./sip-8136.md). The semantics this SIP requires of it are: the bitmap of a row subnet covers the rows mapped to the subnet in `blob_index` order, and within a row follows `cell_index` order; the GroupID is derived from the block root, analogously to column topics. A cell received on a row subnet MUST be verified against the corresponding blob KZG commitment before it is forwarded or used, and peers providing invalid cells MUST be penalized under the same rules as on column subnets.
