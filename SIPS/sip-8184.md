...
discussions-to: https://sila-magicians.org/t/sip-8184-lucid-encrypted-mempool/28017
status: Draft
...
An ST ticket is a new [SIP-2718](./sip-2718.md) typed transaction. It is charged and consumes the sender's execution nonce, but is not executed as a normal SVM message call.
...
Sealed transactions can be bundled tosilaer only if all contained ST tickets have the same `key_publisher`. The bundle is then signed by that `key_publisher` to authenticate that it formed it.
...
* Update `/sila/v2/events` endpoint to notify wallets that their ST has been included, and it is safe to publish the key.
* Update `/sila/v2/events` endpoint to notify clients and committees that a key has been released.
* Add `/sila/v1/beacon/key` endpoint to relay the key to the libp2p topic.
