...
sip: 8200
title: SVMification
description: Replace RIPEMD-160, MODEXP, BLAKE2f, and BLS12 map-to-curve precompiles with equivalent SVM bytecode
author: Kevaundray Wedderburn (@kevaundray)
discussions-to: https://sila-magicians.org/t/sip-8200-evmification/28036
status: Draft
...
- **BLS12_MAP_FP_TO_G1** (`0x10`): Maps a base field element to a G1 point on the BLS12-381 curve. This is pure field arithmetic that can be implemented in SVM.
- **BLS12_MAP_FP2_TO_G2** (`0x11`): Maps an extension field element to a G2 point on the BLS12-381 curve. This is pure field arithmetic that can be implemented in SVM.
...
| `RIPEMD160_SVM_CODE` | <-- TODO: bytecode --> |
| `MODEXP_SVM_CODE` | <-- TODO: bytecode --> |
| `BLAKE2F_SVM_CODE` | <-- TODO: bytecode --> |
| `BLS12_MAP_FP_TO_G1_SVM_CODE` | <-- TODO: bytecode --> |
| `BLS12_MAP_FP2_TO_G2_SVM_CODE` | <-- TODO: bytecode --> |

### Deployment

At the start of the block in which this fork activates, for each precompile address and its corresponding SVM code listed above:

1. Set the code of the address to the corresponding `SVM_CODE`.
...
The current precompile charges `5500` gas. The SVM implementation will charge the sum of the opcodes executed.

<-- TODO: gas comparison with bytecode version -->

#### BLS12_MAP_FP2_TO_G2

The current precompile charges `23800` gas. The SVM implementation will charge the sum of the opcodes executed.
...
This SIP follows the same deployment mechanism established by [SIP-7666](./sip-7666.md): deploying SVM bytecode directly at the precompile address at fork time. This preserves backward compatibility for contracts that call these addresses, as they continue to work with the same interface.
