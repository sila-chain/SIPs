# NTT-SIP as a building block for FALCON, DILITHIUM and STARK verifiers

This repository contains the SIP for NTT transform, along with a python reference code, and a solidity implementation.
...
- **cons**:  requires to be wrapped into  implementations, not optimal for a given target compared to dedicated SIP, not stateless.
...
Further optimizations are reached by using Yul for critical sections and using the CODECOPY and EXTCODECOPY trick detailed in of [[RD23]](https://eprint.iacr.org/2023/939.pdf) (section 3.3, "Hacking SVM memory access cost").
...
ZKNOX is planning a client implementation for node of the considered SIP.

## Conclusion

We provided an optimized version of FALCON, using an optimized version of NTT. This code can be used to speed up Stark verification as well as other lattices primitives (Dilithium, Kyber, etc.). While it seems achievable to use FALCON as a progressive precompile, the cost remains very high. Using a client implementation with NTT-SIP (in a Gsil fork for example), SILA could become from a PQ-Friendly and ZK-Friendly chain. This work is supported by the Sila Foundation.
...
- [[SIP616]](https://sips.sila.org/SIPS/sip-616) SIP-616: SIMD Operations for the SVM. Greg Colvin.
- [[RD23]](https://eprint.iacr.org/2023/939.pdf) Speeding up elliptic computations for Sila Account Abstraction. Renaud Dubois.
