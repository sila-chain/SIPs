from consensus-specs.test.context import (
...
from consensus-specs.test.helpers.constants import MINIMAL
from consensus-specs.test.helpers.withdrawals import (
...
def test_basic_consolidation_in_current_consolidation_epoch(spec, state):
    ...
    # This state has 256 validators each with 32 SIL in MINIMAL preset, 128 SIL consolidation churn
    current_epoch = spec.get_current_epoch(state)
    source_index = spec.get_active_validator_indices(state, current_epoch)[0]
    target_index = spec.get_active_validator_indices(state, current_epoch)[1]

    # Set source to sil1 credentials
    source_address = b"\x22" * 20
    set_sil1_withdrawal_credential_with_balance(spec, state, source_index, address=source_address)
...
def test_incorrect_same_source_target(spec, state):
    ...
    # request, the source validator must be an sil1 validator (0x01).
    set_compounding_withdrawal_credential_with_balance(
...
@spec_test
@single_phase
def test_incorrect_target_with_sil1_credential(spec, state):
    # move state forward SHARD_COMMITTEE_PERIOD epochs to allow for consolidation
    state.slot += spec.config.SHARD_COMMITTEE_PERIOD * spec.SLOTS_PER_EPOCH
    # Set up an otherwise correct consolidation
    current_epoch = spec.get_current_epoch(state)
    ...
    # Set target to sil1 credentials
    set_sil1_withdrawal_credential_with_balance(spec, state, target_index)
...
def test_single_consolidation_request_at_max_eb(spec, state):
    ...
    set_sil1_withdrawal_credential_with_balance(spec, state, another_source_index, address=another_source_address)
...
def test_single_pending_consolidation_exceeding_max_eb(spec, state):
    ...
    another_source_index = spec.get_active_validator_indices(state, current_epoch)[2]
    set_sil1_withdrawal_credential_with_balance(spec, state, another_source_index, address=source_address)
...
def test_multiple_pending_consolidations_at_max_eb(spec, state):
    ...
    set_sil1_withdrawal_credential_with_balance(spec, state, second_source_index, address=source_address)
...
def run_switch_to_compounding_processing(spec, state, consolidation, success=True):
    ...
    if success:
        ...
        assert source_index == target_index
        # Check that the credentials before the switch are of SIL1 type
        assert pre_withdrawal_credentials[:1] == spec.SIL1_ADDRESS_WITHDRAWAL_PREFIX
        # Check source address in the consolidation fits the withdrawal credentials
        assert (
