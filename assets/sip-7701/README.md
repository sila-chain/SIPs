# SIP-7701: Native Account Abstraction explained

The purpose of this document is to provide more details about the design of SIP-7701,
while keeping the [Specification](../../SIPS/sip-7701.md#specification) section of the core document concise.

## Definitions for terms used in SIP-7701

* **Smart Contract Account**: an Sila smart contract that serves as the user's account and on-chain identity.
...
* **Entity**: a common term for any of the smart contracts mentioned above in the context of an SIP-7701 Transaction.
* **Transaction Validity**:
  A property of an Sila transaction that describes whether this transaction can be included in a block without a
  violation of the sila execution and consensus rules.
  This property depends on both the inputs of the transaction and the current state of the Sila blockchain and can
  change over time.
* **SIP-7701 Transaction**: the entire transaction initiated by the `Sender` Smart Contract Account and represented with
  an [SIP-2718](../../SIPS/sip-2718) compatible Transaction Envelope object.
...
* **Top-Level Call Frame**: The initial execution context of a transaction accessing the contract, the "entry point" to
  the SVM code.
* **SIP-7701 Call Frame**:
  A single atomic element of SVM code execution,
  represented by a single top-level call to a specific address with a given data.\
  An SIP-7701 call frame may contain inner call frames as well, but they are not referred to as "SIP-7701 call frames".\
  An SIP-7701 call frame may either succeed or revert.
* **Frame's Role**:
  An identifier of an action that the invoked contract is asked to make during the current call frame.
  An Entity may have one or more roles as part of the SIP-7701 Transaction flow.
* **SIP-7701 Transaction Phase**:
  A set of SIP-7701 Call Frames that form a single step in an SIP-7701 Transaction flow.
  There are two phases in an SIP-7701 Transaction: *validation* and *execution*
* **Validation phase**:
  A set of SIP-7701 Call Frames that define the current SIP-7701 Transaction's **Validity** by executing the
  **validation** SVM code.
* **Execution phase**:
  A set of SIP-7701 Call Frames that perform the actions according to the `Sender` and the `Paymaster` contracts'
...
the SIP-7701 Call Frame reverts.

An SIP-7701 transaction is valid if and only if the following conditions are met for each of
...
When processing an SIP-7701 transaction, however, multiple execution frames will be created.
...
Note that some behaviours in the SVM depend on the transaction context. These behaviours include:

1. Costs of the `SSTORE (0x55)` opcode per [SIP-2200](../../SIPS/sip-2200)
2. Costs of accessing cold addresses and slots per [SIP-2929](../../SIPS/sip-2929)
3. Values available within the transient storage per [SIP-1153](../../SIPS/sip-1153)
4. Maximum amount of gas refund assigned after the execution per [SIP-3529](../../SIPS/sip-3529)
