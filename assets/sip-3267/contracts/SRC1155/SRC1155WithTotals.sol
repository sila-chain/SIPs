...
/// @title A base contract for an SRC-1155 contract with calculation of totals.
abstract contract SRC1155WithTotals is SRC1155 {
    ...
    constructor (string memory uri_) SRC1155(uri_) { }
    ...
    /// Total supply of a token (conforms to `ISRC1155Views`).
    /// @param id Token ID.
    /// @return Total supply.
    ...
    function _batchMint(address to, uint256[] memory ids, uint256[] memory values, bytes memory data) internal {
        require(to != address(0), "SRC1155: batch mint to the zero address");
        require(ids.length == values.length, "SRC1155: IDs and values must have same lengths");
