"""A Yul-subset compiler with two backends: SIP-7979 and legacy SVM.

The point is a demonstration: Yul already has functions, so a
compiler emits them onto CALLSUB / CALLDEST / RETURNSUB nearly
one-to-one, and the output validates under the SIP's reference
...
# to left, so the first argument lands on top, as the SVM pops it.
BUILTINS = {
...
def compile_yul(src, mode="7979"):
    """Compile a Yul-subset source text to SVM bytecode."""
