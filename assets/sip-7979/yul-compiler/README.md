...
SIP.  The same compiler has a legacy backend that emits the same
...
SVM with the three instructions and the runtime checks of the SIP);
...
* `run7979.py` — the interpreter: SVM subset plus `CALLSUB`,
  `CALLDEST`, `RETURNSUB`, with SIP runtime semantics and Yellow
...
* `validator.py`, `opcodes.py` — the validation SIP's reference
