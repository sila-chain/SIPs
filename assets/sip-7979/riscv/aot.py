"""Ahead-of-time translation of validated SVM code to RISC-V.
...
known inputs and net stack effect — and spends those proofs:
  ...
  * Dynamic dispatch disappears: JUMP is `j`, JUMPI is `bnez`,
    CALLSUB is `jal`, RETURNSUB is `ret`.  The SVM return stack
  ...
  * No underflow or destination checks survive into the output,
    ...
    the SIP says it must: one compare per call, none per instruction.
...
def annotate(code):
    ...
    the translator's subset (which would still be valid SIP-7979)."""
    n = Notes()
...
class Translator:
    ...
    def gas_check(self, amount):
        ...
        the metering a sila-mainnet AOT cannot omit."""
